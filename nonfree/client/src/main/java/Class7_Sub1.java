import java.awt.Component;
import javax.sound.sampled.AudioFormat;
import javax.sound.sampled.AudioSystem;
import javax.sound.sampled.DataLine.Info;
import javax.sound.sampled.LineUnavailableException;
import javax.sound.sampled.SourceDataLine;
import org.openrs2.deob.annotation.OriginalArg;
import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;
import org.openrs2.deob.annotation.Pc;

@OriginalClass("client!ah")
public final class Class7_Sub1 extends Class7 {

	@OriginalMember(owner = "client!ah", name = "J", descriptor = "Ljavax/sound/sampled/SourceDataLine;")
	private SourceDataLine aSourceDataLine1;

	@OriginalMember(owner = "client!ah", name = "K", descriptor = "Ljavax/sound/sampled/AudioFormat;")
	private AudioFormat anAudioFormat1;

	@OriginalMember(owner = "client!ah", name = "M", descriptor = "I")
	private int anInt255;

	@OriginalMember(owner = "client!ah", name = "N", descriptor = "[B")
	private byte[] aByteArray2;

	@OriginalMember(owner = "client!ah", name = "L", descriptor = "Z")
	private boolean aBoolean24 = false;

	@OriginalMember(owner = "client!ah", name = "a", descriptor = "()I")
	@Override
	protected int method5546() {
		return this.anInt255 - (this.aSourceDataLine1.available() >> (Static330.aBoolean615 ? 2 : 1));
	}

	@OriginalMember(owner = "client!ah", name = "a", descriptor = "(I)V")
	@Override
	public void method5544(@OriginalArg(0) int arg0) throws LineUnavailableException {
		try {
			@Pc(20) Info local20 = new Info(SourceDataLine.class, this.anAudioFormat1, arg0 << (Static330.aBoolean615 ? 2 : 1));
			this.aSourceDataLine1 = (SourceDataLine) AudioSystem.getLine(local20);
			this.aSourceDataLine1.open();
			this.aSourceDataLine1.start();
			this.anInt255 = arg0;
		} catch (@Pc(36) LineUnavailableException local36) {
			if (Static354.method5634(arg0) == 1) {
				this.aSourceDataLine1 = null;
				throw local36;
			} else {
				this.method5544(Static194.method3745(arg0));
			}
		}
	}

	@OriginalMember(owner = "client!ah", name = "c", descriptor = "()V")
	@Override
	protected void method5550() {
		if (this.aSourceDataLine1 != null) {
			this.aSourceDataLine1.close();
			this.aSourceDataLine1 = null;
		}
	}

	@OriginalMember(owner = "client!ah", name = "d", descriptor = "()V")
	@Override
	protected void method5557() throws LineUnavailableException {
		this.aSourceDataLine1.flush();
		if (!this.aBoolean24) {
			return;
		}
		this.aSourceDataLine1.close();
		this.aSourceDataLine1 = null;
		@Pc(34) Info local34 = new Info(SourceDataLine.class, this.anAudioFormat1, this.anInt255 << (Static330.aBoolean615 ? 2 : 1));
		this.aSourceDataLine1 = (SourceDataLine) AudioSystem.getLine(local34);
		this.aSourceDataLine1.open();
		this.aSourceDataLine1.start();
	}

	@OriginalMember(owner = "client!ah", name = "a", descriptor = "(Ljava/awt/Component;)V")
	@Override
	public void method5554(@OriginalArg(0) Component arg0) {
		@Pc(1) javax.sound.sampled.Mixer.Info[] local1 = AudioSystem.getMixerInfo();
		if (local1 != null) {
			for (@Pc(9) int local9 = 0; local9 < local1.length; local9++) {
				@Pc(17) javax.sound.sampled.Mixer.Info local17 = local1[local9];
				if (local17 != null) {
					@Pc(22) String local22 = local17.getName();
					if (local22 != null && local22.toLowerCase().indexOf("soundmax") >= 0) {
						this.aBoolean24 = true;
					}
				}
			}
		}
		this.anAudioFormat1 = new AudioFormat((float) Static38.anInt1250, 16, Static330.aBoolean615 ? 2 : 1, true, false);
		this.aByteArray2 = new byte[0x100 << (Static330.aBoolean615 ? 2 : 1)];
	}

	@OriginalMember(owner = "client!ah", name = "b", descriptor = "()V")
	@Override
	protected void method5547() {
		@Pc(1) short local1 = 256;
		if (Static330.aBoolean615) {
			local1 = 512;
		}
		for (@Pc(9) int local9 = 0; local9 < local1; local9++) {
			@Pc(17) int local17 = this.anIntArray527[local9];
			if ((local17 + 8388608 & 0xFF000000) != 0) {
				local17 = local17 >> 31 ^ 0x7FFFFF;
			}
			this.aByteArray2[local9 * 2] = (byte) (local17 >> 8);
			this.aByteArray2[local9 * 2 + 1] = (byte) (local17 >> 16);
		}
		this.aSourceDataLine1.write(this.aByteArray2, 0, local1 << 1);
	}
}
