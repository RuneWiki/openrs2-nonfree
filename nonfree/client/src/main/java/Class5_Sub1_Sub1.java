import java.io.ByteArrayInputStream;
import javax.sound.midi.InvalidMidiDataException;
import javax.sound.midi.MidiMessage;
import javax.sound.midi.MidiSystem;
import javax.sound.midi.Receiver;
import javax.sound.midi.Sequence;
import javax.sound.midi.ShortMessage;
import org.openrs2.deob.annotation.OriginalArg;
import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;
import org.openrs2.deob.annotation.Pc;

@OriginalClass("client!qe")
public final class Class5_Sub1_Sub1 extends Class5_Sub1 implements Receiver {

	@OriginalMember(owner = "client!qe", name = "<init>", descriptor = "()V")
	public Class5_Sub1_Sub1() {
		try {
			Static82.aReceiver1 = MidiSystem.getReceiver();
			Static82.aSequencer1 = MidiSystem.getSequencer(false);
			Static82.aSequencer1.getTransmitter().setReceiver(this);
			Static82.aSequencer1.open();
			this.method1611(-1L);
		} catch (@Pc(18) Exception local18) {
			Static29.method744();
		}
	}

	@OriginalMember(owner = "client!qe", name = "send", descriptor = "(Ljavax/sound/midi/MidiMessage;J)V")
	@Override
	public synchronized void send(@OriginalArg(0) MidiMessage arg0, @OriginalArg(1) long arg1) {
		if (Static82.aBoolean110) {
			@Pc(5) byte[] local5 = arg0.getMessage();
			if (!this.method1614(local5[0] & 0xFF, local5[1], local5.length >= 3 ? local5[2] : 0, arg1)) {
				Static82.aReceiver1.send(arg0, arg1);
			}
		}
	}

	@OriginalMember(owner = "client!qe", name = "a", descriptor = "([BZII)V")
	@Override
	public void method1600(@OriginalArg(0) byte[] arg0, @OriginalArg(1) boolean arg1, @OriginalArg(2) int arg2) {
		if (Static82.aSequencer1 == null) {
			return;
		}
		try {
			@Pc(8) Sequence local8 = MidiSystem.getSequence(new ByteArrayInputStream(arg0));
			Static82.aSequencer1.setSequence(local8);
			Static82.aSequencer1.setLoopCount(arg1 ? -1 : 0);
			this.method1609(0, -1L, arg2);
			Static82.aBoolean110 = true;
			Static82.aSequencer1.start();
		} catch (@Pc(38) Exception local38) {
		}
	}

	@OriginalMember(owner = "client!qe", name = "a", descriptor = "(B)V")
	@Override
	public void method1606() {
		if (Static82.aSequencer1 != null) {
			Static82.aBoolean110 = false;
			Static82.aSequencer1.stop();
			this.method1611(-1L);
		}
	}

	@OriginalMember(owner = "client!qe", name = "d", descriptor = "(I)V")
	@Override
	public void method1607() {
		if (Static82.aSequencer1 != null) {
			Static82.aSequencer1.close();
			Static82.aSequencer1 = null;
		}
		if (Static82.aReceiver1 != null) {
			Static82.aReceiver1.close();
			Static82.aReceiver1 = null;
		}
	}

	@OriginalMember(owner = "client!qe", name = "a", descriptor = "(IZI)V")
	@Override
	public synchronized void method1604(@OriginalArg(0) int arg0, @OriginalArg(2) int arg1) {
		if (Static82.aSequencer1 != null) {
			this.method1609(arg1, -1L, arg0);
		}
	}

	@OriginalMember(owner = "client!qe", name = "a", descriptor = "(IZ)V")
	@Override
	public void method1602(@OriginalArg(0) int arg0) {
		if (Static82.aSequencer1 != null) {
			this.method1617(arg0, -1L);
		}
	}

	@OriginalMember(owner = "client!qe", name = "b", descriptor = "(IIIJ)V")
	@Override
	protected void method1616(@OriginalArg(0) int arg0, @OriginalArg(1) int arg1, @OriginalArg(2) int arg2, @OriginalArg(3) long arg3) {
		try {
			@Pc(3) ShortMessage local3 = new ShortMessage();
			local3.setMessage(arg0, arg1, arg2);
			Static82.aReceiver1.send(local3, arg3);
		} catch (@Pc(14) InvalidMidiDataException local14) {
		}
	}

	@OriginalMember(owner = "client!qe", name = "close", descriptor = "()V")
	@Override
	public void close() {
	}

	@OriginalMember(owner = "client!qe", name = "a", descriptor = "(Z)V")
	@Override
	public void method1598() {
	}
}
