import java.awt.Component;
import javax.sound.sampled.AudioFormat;
import javax.sound.sampled.AudioSystem;
import javax.sound.sampled.DataLine.Info;
import javax.sound.sampled.LineUnavailableException;
import javax.sound.sampled.SourceDataLine;
import org.openrs2.deob.annotation.OriginalArg;
import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;
import org.openrs2.deob.annotation.Pc;

@OriginalClass("client!pf")
public final class Class8_Sub2 extends Class8 {

	@OriginalMember(owner = "client!pf", name = "L", descriptor = "Ljavax/sound/sampled/AudioFormat;")
	private AudioFormat anAudioFormat1;

	@OriginalMember(owner = "client!pf", name = "M", descriptor = "[B")
	private byte[] aByteArray40;

	@OriginalMember(owner = "client!pf", name = "N", descriptor = "I")
	private int anInt3635;

	@OriginalMember(owner = "client!pf", name = "P", descriptor = "Ljavax/sound/sampled/SourceDataLine;")
	private SourceDataLine aSourceDataLine1;

	@OriginalMember(owner = "client!pf", name = "O", descriptor = "Z")
	private boolean aBoolean138 = false;

	@OriginalMember(owner = "client!pf", name = "c", descriptor = "()V")
	@Override
	protected void method2798() {
		if (this.aSourceDataLine1 != null) {
			this.aSourceDataLine1.close();
			this.aSourceDataLine1 = null;
		}
	}

	@OriginalMember(owner = "client!pf", name = "c", descriptor = "(I)V")
	@Override
	public void method2800(@OriginalArg(0) int arg0) throws LineUnavailableException {
		try {
			@Pc(20) Info local20 = new Info(SourceDataLine.class, this.anAudioFormat1, arg0 << (Static16.aBoolean15 ? 2 : 1));
			this.aSourceDataLine1 = (SourceDataLine) AudioSystem.getLine(local20);
			this.aSourceDataLine1.open();
			this.aSourceDataLine1.start();
			this.anInt3635 = arg0;
		} catch (@Pc(36) LineUnavailableException local36) {
			if (Static22.method546(arg0) == 1) {
				this.aSourceDataLine1 = null;
				throw local36;
			} else {
				this.method2800(Static150.method2965(arg0));
			}
		}
	}

	@OriginalMember(owner = "client!pf", name = "d", descriptor = "()V")
	@Override
	protected void method2802() throws LineUnavailableException {
		this.aSourceDataLine1.flush();
		if (!this.aBoolean138) {
			return;
		}
		this.aSourceDataLine1.close();
		this.aSourceDataLine1 = null;
		@Pc(33) Info local33 = new Info(SourceDataLine.class, this.anAudioFormat1, this.anInt3635 << (Static16.aBoolean15 ? 2 : 1));
		this.aSourceDataLine1 = (SourceDataLine) AudioSystem.getLine(local33);
		this.aSourceDataLine1.open();
		this.aSourceDataLine1.start();
	}

	@OriginalMember(owner = "client!pf", name = "a", descriptor = "()I")
	@Override
	protected int method2789() {
		return this.anInt3635 - (this.aSourceDataLine1.available() >> (Static16.aBoolean15 ? 2 : 1));
	}

	@OriginalMember(owner = "client!pf", name = "b", descriptor = "()V")
	@Override
	protected void method2792() {
		@Pc(1) short local1 = 256;
		if (Static16.aBoolean15) {
			local1 = 512;
		}
		for (@Pc(9) int local9 = 0; local9 < local1; local9++) {
			@Pc(17) int local17 = this.anIntArray410[local9];
			if ((local17 + 8388608 & 0xFF000000) != 0) {
				local17 = local17 >> 31 ^ 0x7FFFFF;
			}
			this.aByteArray40[local9 * 2] = (byte) (local17 >> 8);
			this.aByteArray40[local9 * 2 + 1] = (byte) (local17 >> 16);
		}
		this.aSourceDataLine1.write(this.aByteArray40, 0, local1 << 1);
	}

	@OriginalMember(owner = "client!pf", name = "a", descriptor = "(Ljava/awt/Component;)V")
	@Override
	public void method2799(@OriginalArg(0) Component arg0) {
		@Pc(1) javax.sound.sampled.Mixer.Info[] local1 = AudioSystem.getMixerInfo();
		if (local1 != null) {
			for (@Pc(6) int local6 = 0; local6 < local1.length; local6++) {
				@Pc(20) javax.sound.sampled.Mixer.Info local20 = local1[local6];
				if (local20 != null) {
					@Pc(27) String local27 = local20.getName();
					if (local27 != null && local27.toLowerCase().indexOf("soundmax") >= 0) {
						this.aBoolean138 = true;
					}
				}
			}
		}
		this.anAudioFormat1 = new AudioFormat((float) Static98.anInt2858, 16, Static16.aBoolean15 ? 2 : 1, true, false);
		this.aByteArray40 = new byte[0x100 << (Static16.aBoolean15 ? 2 : 1)];
	}
}
