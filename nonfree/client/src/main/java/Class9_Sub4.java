import org.openrs2.deob.annotation.OriginalArg;
import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;

@OriginalClass("client!ku")
public final class Class9_Sub4 extends Class9 {

	@OriginalMember(owner = "client!ku", name = "n", descriptor = "B")
	public final byte aByte77;

	@OriginalMember(owner = "client!ku", name = "p", descriptor = "B")
	public final byte aByte74;

	@OriginalMember(owner = "client!ku", name = "m", descriptor = "I")
	public int anInt5627;

	@OriginalMember(owner = "client!ku", name = "k", descriptor = "B")
	public final byte aByte75;

	@OriginalMember(owner = "client!ku", name = "l", descriptor = "B")
	public final byte aByte76;

	@OriginalMember(owner = "client!ku", name = "o", descriptor = "[S")
	public final short[] aShortArray67;

	@OriginalMember(owner = "client!ku", name = "q", descriptor = "[B")
	public final byte[] aByteArray53;

	@OriginalMember(owner = "client!ku", name = "<init>", descriptor = "(IIIII[S[B)V")
	public Class9_Sub4(@OriginalArg(0) int arg0, @OriginalArg(1) int arg1, @OriginalArg(2) int arg2, @OriginalArg(3) int arg3, @OriginalArg(4) int arg4, @OriginalArg(5) short[] arg5, @OriginalArg(6) byte[] arg6) {
		this.aByte77 = (byte) arg0;
		this.aByte74 = (byte) arg1;
		this.anInt5627 = arg2;
		this.aByte75 = (byte) arg3;
		this.aByte76 = (byte) arg4;
		this.aShortArray67 = arg5;
		this.aByteArray53 = arg6;
	}
}
