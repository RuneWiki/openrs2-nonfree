import java.awt.Component;
import javax.sound.sampled.AudioFormat;
import javax.sound.sampled.AudioSystem;
import javax.sound.sampled.DataLine.Info;
import javax.sound.sampled.LineUnavailableException;
import javax.sound.sampled.SourceDataLine;
import org.openrs2.deob.annotation.OriginalArg;
import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;
import org.openrs2.deob.annotation.Pc;

@OriginalClass("client!ia")
public final class Class34_Sub1 extends Class34 {

	@OriginalMember(owner = "client!ia", name = "N", descriptor = "Ljavax/sound/sampled/SourceDataLine;")
	private SourceDataLine aSourceDataLine1;

	@OriginalMember(owner = "client!ia", name = "O", descriptor = "I")
	private int anInt1264;

	@OriginalMember(owner = "client!ia", name = "P", descriptor = "Ljavax/sound/sampled/AudioFormat;")
	private AudioFormat anAudioFormat1;

	@OriginalMember(owner = "client!ia", name = "Q", descriptor = "[B")
	private byte[] aByteArray17;

	@OriginalMember(owner = "client!ia", name = "d", descriptor = "()V")
	@Override
	protected void method2121() {
		if (this.aSourceDataLine1 != null) {
			this.aSourceDataLine1.close();
			this.aSourceDataLine1 = null;
		}
	}

	@OriginalMember(owner = "client!ia", name = "c", descriptor = "()I")
	@Override
	protected int method2119() {
		return this.anInt1264 - (this.aSourceDataLine1.available() >> (Static34.aBoolean59 ? 2 : 1));
	}

	@OriginalMember(owner = "client!ia", name = "a", descriptor = "(Ljava/awt/Component;)V")
	@Override
	public void method2120(@OriginalArg(0) Component arg0) {
		this.anAudioFormat1 = new AudioFormat((float) Static68.anInt1863, 16, Static34.aBoolean59 ? 2 : 1, true, false);
		this.aByteArray17 = new byte[0x100 << (Static34.aBoolean59 ? 2 : 1)];
	}

	@OriginalMember(owner = "client!ia", name = "a", descriptor = "()V")
	@Override
	protected void method2116() {
		this.aSourceDataLine1.flush();
	}

	@OriginalMember(owner = "client!ia", name = "f", descriptor = "(I)V")
	@Override
	public void method2124(@OriginalArg(0) int arg0) throws LineUnavailableException {
		try {
			@Pc(20) Info local20 = new Info(SourceDataLine.class, this.anAudioFormat1, arg0 << (Static34.aBoolean59 ? 2 : 1));
			this.aSourceDataLine1 = (SourceDataLine) AudioSystem.getLine(local20);
			this.aSourceDataLine1.open();
			this.aSourceDataLine1.start();
			this.anInt1264 = arg0;
		} catch (@Pc(36) LineUnavailableException local36) {
			if (Static84.method1581(arg0) == 1) {
				this.aSourceDataLine1 = null;
				throw local36;
			} else {
				this.method2124(Static35.method530(arg0));
			}
		}
	}

	@OriginalMember(owner = "client!ia", name = "b", descriptor = "()V")
	@Override
	protected void method2118() {
		@Pc(1) short local1 = 256;
		if (Static34.aBoolean59) {
			local1 = 512;
		}
		for (@Pc(9) int local9 = 0; local9 < local1; local9++) {
			@Pc(17) int local17 = this.anIntArray351[local9];
			if ((local17 + 8388608 & 0xFF000000) != 0) {
				local17 = local17 >> 31 ^ 0x7FFFFF;
			}
			this.aByteArray17[local9 * 2] = (byte) (local17 >> 8);
			this.aByteArray17[local9 * 2 + 1] = (byte) (local17 >> 16);
		}
		this.aSourceDataLine1.write(this.aByteArray17, 0, local1 << 1);
	}
}
