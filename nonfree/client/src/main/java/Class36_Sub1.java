import java.awt.Component;
import java.awt.event.MouseWheelEvent;
import java.awt.event.MouseWheelListener;
import org.openrs2.deob.annotation.OriginalArg;
import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;
import org.openrs2.deob.annotation.Pc;

@OriginalClass("client!hg")
public final class Class36_Sub1 extends Class36 implements MouseWheelListener {

	@OriginalMember(owner = "client!hg", name = "q", descriptor = "I")
	private int anInt2187 = 0;

	@OriginalMember(owner = "client!hg", name = "a", descriptor = "(BLjava/awt/Component;)V")
	@Override
	public void method1610(@OriginalArg(1) Component arg0) {
		arg0.removeMouseWheelListener(this);
	}

	@OriginalMember(owner = "client!hg", name = "a", descriptor = "(ILjava/awt/Component;)V")
	@Override
	public void method1612(@OriginalArg(1) Component arg0) {
		arg0.addMouseWheelListener(this);
	}

	@OriginalMember(owner = "client!hg", name = "mouseWheelMoved", descriptor = "(Ljava/awt/event/MouseWheelEvent;)V")
	@Override
	public synchronized void mouseWheelMoved(@OriginalArg(0) MouseWheelEvent arg0) {
		this.anInt2187 += arg0.getWheelRotation();
	}

	@OriginalMember(owner = "client!hg", name = "a", descriptor = "(B)I")
	@Override
	public synchronized int method1611() {
		@Pc(10) int local10 = this.anInt2187;
		this.anInt2187 = 0;
		return local10;
	}
}
