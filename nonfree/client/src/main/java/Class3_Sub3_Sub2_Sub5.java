import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;
import org.openrs2.deob.annotation.Pc;

@OriginalClass("client!sg")
public final class Class3_Sub3_Sub2_Sub5 extends Class3_Sub3_Sub2 {

	@OriginalMember(owner = "client!sg", name = "V", descriptor = "[I")
	private static final int[] anIntArray301 = new int[512];

	@OriginalMember(owner = "client!sg", name = "Y", descriptor = "[I")
	public static final int[] anIntArray303 = new int[2048];

	@OriginalMember(owner = "client!sg", name = "S", descriptor = "[I")
	public static final int[] anIntArray300 = new int[2048];

	@OriginalMember(owner = "client!sg", name = "bb", descriptor = "[I")
	public static final int[] anIntArray304 = new int[2048];

	static {
		for (@Pc(31) int local31 = 1; local31 < 512; local31++) {
			anIntArray301[local31] = 32768 / local31;
		}
		for (@Pc(44) int local44 = 1; local44 < 2048; local44++) {
			anIntArray304[local44] = 65536 / local44;
		}
		for (@Pc(57) int local57 = 0; local57 < 2048; local57++) {
			anIntArray300[local57] = (int) (Math.sin((double) local57 * 0.0030679615D) * 65536.0D);
			anIntArray303[local57] = (int) (Math.cos((double) local57 * 0.0030679615D) * 65536.0D);
		}
	}
}
