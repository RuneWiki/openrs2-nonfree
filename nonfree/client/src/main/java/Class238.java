import org.openrs2.deob.annotation.OriginalArg;
import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;
import org.openrs2.deob.annotation.Pc;

@OriginalClass("client!uh")
public final class Class238 {

	@OriginalMember(owner = "client!uh", name = "a", descriptor = "I")
	private int anInt6984;

	@OriginalMember(owner = "client!uh", name = "b", descriptor = "I")
	private int anInt6985;

	@OriginalMember(owner = "client!uh", name = "c", descriptor = "I")
	private int anInt6986;

	@OriginalMember(owner = "client!uh", name = "e", descriptor = "[I")
	private int[] anIntArray579;

	@OriginalMember(owner = "client!uh", name = "k", descriptor = "I")
	private int anInt6992;

	@OriginalMember(owner = "client!uh", name = "n", descriptor = "[I")
	private int[] anIntArray580;

	@OriginalMember(owner = "client!uh", name = "<init>", descriptor = "()V")
	private Class238() {
	}

	@OriginalMember(owner = "client!uh", name = "<init>", descriptor = "([I)V")
	public Class238(@OriginalArg(0) int[] arg0) {
		this.anIntArray579 = new int[256];
		this.anIntArray580 = new int[256];
		for (@Pc(13) int local13 = 0; local13 < arg0.length; local13++) {
			this.anIntArray580[local13] = arg0[local13];
		}
		this.method5460();
	}

	@OriginalMember(owner = "client!uh", name = "a", descriptor = "(I)I")
	public int method5457() {
		if (this.anInt6992 == 0) {
			this.method5459();
			this.anInt6992 = 256;
		}
		return this.anIntArray580[this.anInt6992 - 1];
	}

	@OriginalMember(owner = "client!uh", name = "a", descriptor = "(Z)I")
	public int method5458() {
		if (this.anInt6992 == 0) {
			this.method5459();
			this.anInt6992 = 256;
		}
		return this.anIntArray580[--this.anInt6992];
	}

	@OriginalMember(owner = "client!uh", name = "a", descriptor = "(B)V")
	private void method5459() {
		this.anInt6985 += ++this.anInt6986;
		for (@Pc(23) int local23 = 0; local23 < 256; local23++) {
			@Pc(30) int local30 = this.anIntArray579[local23];
			if ((local23 & 0x2) == 0) {
				if ((local23 & 0x1) == 0) {
					this.anInt6984 ^= this.anInt6984 << 13;
				} else {
					this.anInt6984 ^= this.anInt6984 >>> 6;
				}
			} else if ((local23 & 0x1) == 0) {
				this.anInt6984 ^= this.anInt6984 << 2;
			} else {
				this.anInt6984 ^= this.anInt6984 >>> 16;
			}
			this.anInt6984 += this.anIntArray579[local23 + 128 & 0xFF];
			@Pc(125) int local125;
			this.anIntArray579[local23] = local125 = this.anInt6985 + this.anIntArray579[local30 >> 2 & 0xFF] + this.anInt6984;
			this.anIntArray580[local23] = this.anInt6985 = local30 + this.anIntArray579[local125 >> 8 >> 2 & 0xFF];
		}
	}

	@OriginalMember(owner = "client!uh", name = "b", descriptor = "(B)V")
	private void method5460() {
		@Pc(8) int local8 = -1640531527;
		@Pc(10) int local10 = -1640531527;
		@Pc(12) int local12 = -1640531527;
		@Pc(14) int local14 = -1640531527;
		@Pc(16) int local16 = -1640531527;
		@Pc(18) int local18 = -1640531527;
		@Pc(20) int local20 = -1640531527;
		@Pc(21) int local21 = -1640531527;
		@Pc(23) int local23;
		for (local23 = 0; local23 < 4; local23++) {
			local21 ^= local20 << 11;
			local16 += local21;
			local20 += local18;
			local20 ^= local18 >>> 2;
			local14 += local20;
			local18 += local16;
			local18 ^= local16 << 8;
			local12 += local18;
			local16 += local14;
			local16 ^= local14 >>> 16;
			local14 += local12;
			local10 += local16;
			local14 ^= local12 << 10;
			local12 += local10;
			local8 += local14;
			local12 ^= local10 >>> 4;
			local21 += local12;
			local10 += local8;
			local10 ^= local8 << 8;
			local8 += local21;
			local20 += local10;
			local8 ^= local21 >>> 9;
			local18 += local8;
			local21 += local20;
		}
		for (local23 = 0; local23 < 256; local23 += 8) {
			local18 += this.anIntArray580[local23 + 2];
			local21 += this.anIntArray580[local23];
			local12 += this.anIntArray580[local23 + 5];
			local20 += this.anIntArray580[local23 + 1];
			local10 += this.anIntArray580[local23 + 6];
			local16 += this.anIntArray580[local23 + 3];
			local8 += this.anIntArray580[local23 + 7];
			local14 += this.anIntArray580[local23 + 4];
			local21 ^= local20 << 11;
			local16 += local21;
			local20 += local18;
			local20 ^= local18 >>> 2;
			local14 += local20;
			local18 += local16;
			local18 ^= local16 << 8;
			local12 += local18;
			local16 += local14;
			local16 ^= local14 >>> 16;
			local14 += local12;
			local10 += local16;
			local14 ^= local12 << 10;
			local12 += local10;
			local8 += local14;
			local12 ^= local10 >>> 4;
			local21 += local12;
			local10 += local8;
			local10 ^= local8 << 8;
			local8 += local21;
			local20 += local10;
			local8 ^= local21 >>> 9;
			local18 += local8;
			local21 += local20;
			this.anIntArray579[local23] = local21;
			this.anIntArray579[local23 + 1] = local20;
			this.anIntArray579[local23 + 2] = local18;
			this.anIntArray579[local23 + 3] = local16;
			this.anIntArray579[local23 + 4] = local14;
			this.anIntArray579[local23 + 5] = local12;
			this.anIntArray579[local23 + 6] = local10;
			this.anIntArray579[local23 + 7] = local8;
		}
		for (local23 = 0; local23 < 256; local23 += 8) {
			local10 += this.anIntArray579[local23 + 6];
			local21 += this.anIntArray579[local23];
			local16 += this.anIntArray579[local23 + 3];
			local8 += this.anIntArray579[local23 + 7];
			local20 += this.anIntArray579[local23 + 1];
			local14 += this.anIntArray579[local23 + 4];
			local12 += this.anIntArray579[local23 + 5];
			local18 += this.anIntArray579[local23 + 2];
			local21 ^= local20 << 11;
			local16 += local21;
			local20 += local18;
			local20 ^= local18 >>> 2;
			local14 += local20;
			local18 += local16;
			local18 ^= local16 << 8;
			local16 += local14;
			local12 += local18;
			local16 ^= local14 >>> 16;
			local14 += local12;
			local10 += local16;
			local14 ^= local12 << 10;
			local12 += local10;
			local8 += local14;
			local12 ^= local10 >>> 4;
			local21 += local12;
			local10 += local8;
			local10 ^= local8 << 8;
			local8 += local21;
			local20 += local10;
			local8 ^= local21 >>> 9;
			local21 += local20;
			local18 += local8;
			this.anIntArray579[local23] = local21;
			this.anIntArray579[local23 + 1] = local20;
			this.anIntArray579[local23 + 2] = local18;
			this.anIntArray579[local23 + 3] = local16;
			this.anIntArray579[local23 + 4] = local14;
			this.anIntArray579[local23 + 5] = local12;
			this.anIntArray579[local23 + 6] = local10;
			this.anIntArray579[local23 + 7] = local8;
		}
		this.method5459();
		this.anInt6992 = 256;
	}
}
