...
public final class context {
	...
	@OriginalMember(owner = "client!jaggl/context", name = "b", descriptor = "Lclient!jaggl/opengl;")
	private final opengl b = new opengl(this);
	...
	public boolean isExtensionAvailable(@OriginalArg(0) String arg0) {
		if (this.c == null) {
			@Pc(19) String local19 = this.b.glGetString(7939) + " " + this.getExtensionsString();
			@Pc(23) String[] local23 = local19.split(" ");
			this.c = new HashSet(local23.length);
			for (@Pc(32) int local32 = 0; local32 < local23.length; local32++) {
				this.c.add(local23[local32]);
			}
		}
