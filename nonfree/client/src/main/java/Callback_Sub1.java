import com.ms.awt.WComponentPeer;
import com.ms.dll.Callback;
import com.ms.dll.Root;
import com.ms.win32.User32;
import java.awt.Component;
import org.openrs2.deob.annotation.OriginalArg;
import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;
import org.openrs2.deob.annotation.Pc;

@OriginalClass("client!kfa")
public final class Callback_Sub1 extends Callback {

	@OriginalMember(owner = "client!kfa", name = "a", descriptor = "I")
	private volatile int anInt5271;

	@OriginalMember(owner = "client!kfa", name = "b", descriptor = "I")
	private int anInt5272;

	@OriginalMember(owner = "client!kfa", name = "c", descriptor = "I")
	private volatile int anInt5273;

	@OriginalMember(owner = "client!kfa", name = "d", descriptor = "Z")
	private boolean aBoolean392;

	@OriginalMember(owner = "client!kfa", name = "e", descriptor = "Z")
	private volatile boolean aBoolean393 = true;

	@OriginalMember(owner = "client!kfa", name = "a", descriptor = "(ZZLjava/awt/Component;)V")
	public void method4619(@OriginalArg(1) boolean arg0, @OriginalArg(2) Component arg1) {
		@Pc(3) WComponentPeer local3 = (WComponentPeer) arg1.getPeer();
		@Pc(6) int local6 = local3.getTopHwnd();
		if (this.anInt5273 == local6 && this.aBoolean393 == arg0) {
			return;
		}
		if (!this.aBoolean392) {
			this.anInt5272 = User32.LoadCursor(0, 32512);
			Root.alloc(this);
			this.aBoolean392 = true;
		}
		if (this.anInt5273 != local6) {
			if (this.anInt5273 != 0) {
				this.aBoolean393 = true;
				User32.SendMessage(local6, 101024, 0, 0);
				synchronized (this) {
					User32.SetWindowLong(this.anInt5273, -4, this.anInt5271);
				}
			}
			synchronized (this) {
				this.anInt5273 = local6;
				this.anInt5271 = User32.SetWindowLong(this.anInt5273, -4, this);
			}
		}
		this.aBoolean393 = arg0;
		User32.SendMessage(local6, 101024, 0, 0);
	}

	@OriginalMember(owner = "client!kfa", name = "a", descriptor = "(III)V")
	public void method4620(@OriginalArg(1) int arg0, @OriginalArg(2) int arg1) {
		User32.SetCursorPos(arg1, arg0);
	}

	@OriginalMember(owner = "client!kfa", name = "callback", descriptor = "(IIII)I")
	private synchronized int callback(@OriginalArg(0) int arg0, @OriginalArg(1) int arg1, @OriginalArg(2) int arg2, @OriginalArg(3) int arg3) {
		@Pc(11) int local11;
		if (arg0 != this.anInt5273) {
			local11 = User32.GetWindowLong(arg0, -4);
			return User32.CallWindowProc(local11, arg0, arg1, arg2, arg3);
		}
		if (arg1 == 32) {
			local11 = arg3 & 0xFFFF;
			if (local11 == 1) {
				User32.SetCursor(this.aBoolean393 ? this.anInt5272 : 0);
				return 0;
			}
		}
		if (arg1 == 101024) {
			User32.SetCursor(this.aBoolean393 ? this.anInt5272 : 0);
			return 0;
		}
		if (arg1 == 1) {
			this.anInt5273 = 0;
			this.aBoolean393 = true;
		}
		return User32.CallWindowProc(this.anInt5271, arg0, arg1, arg2, arg3);
	}
}
