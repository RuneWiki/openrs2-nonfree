...
public final class Class6_Sub1 extends Class6 {
	...
	private Socket httpProxyConnect(@OriginalArg(0) String arg0, @OriginalArg(1) int arg1, @OriginalArg(2) String arg2) throws IOException {
		...
		if (arg2 == null) {
			local11.write(("CONNECT " + this.aString229 + ":" + this.anInt10337 + " HTTP/1.0\n\n").getBytes(Charset.forName("ISO-8859-1")));
		} else {
			local11.write(("CONNECT " + this.aString229 + ":" + this.anInt10337 + " HTTP/1.0\n" + arg2 + "\n\n").getBytes(Charset.forName("ISO-8859-1")));
		}
		local11.flush();
	...
	private Socket method126(@OriginalArg(0) Proxy arg0) throws IOException {
		...
		@Pc(27) InetSocketAddress local27 = (InetSocketAddress) local18;
		if (arg0.type() == Type.HTTP) {
			...
			try {
				...
				@Pc(99) Object local99 = local77.invoke(null, local27.getHostName(), Integer.valueOf(local27.getPort()));
				if (local99 != null) {
					...
					if ((Boolean) local108.invoke(local99)) {
						...
						@Pc(194) String local194 = (String) local156.invoke(local99, new URL("https://" + this.aString229 + "/"), "https");
			...
			local40.connect(new InetSocketAddress(this.aString229, this.anInt10337));
			return local40;
		} else {
			return null;
		}
	...
	public Socket method8073() throws IOException {
		...
		@Pc(24) boolean local24 = this.anInt10337 == 443;
		@Pc(46) List local46;
		@Pc(68) List local68;
		try {
			local46 = this.aProxySelector1.select(new URI((local24 ? "https" : "http") + "://" + this.aString229));
			local68 = this.aProxySelector1.select(new URI((local24 ? "http" : "https") + "://" + this.aString229));
		} catch (@Pc(70) URISyntaxException local70) {
			return this.method8074();
		}
		local46.addAll(local68);
		@Pc(83) Object[] local83 = local46.toArray();
		...
		for (@Pc(89) int local89 = 0; local89 < local83.length; local89++) {
			...
			try {
				@Pc(105) Socket local105 = this.method126(local100);
				if (local105 != null) {
					return local105;
				}
