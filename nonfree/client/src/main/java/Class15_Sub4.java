import jaggl.opengl;
import java.nio.ByteBuffer;
import org.openrs2.deob.annotation.OriginalArg;
import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;
import org.openrs2.deob.annotation.Pc;

@OriginalClass("client!wq")
public final class Class15_Sub4 extends Class15 {

	@OriginalMember(owner = "client!wq", name = "i", descriptor = "I")
	private final int anInt6391;

	@OriginalMember(owner = "client!wq", name = "<init>", descriptor = "(Lclient!nm;II[BI)V")
	public Class15_Sub4(@OriginalArg(0) Class164_Sub1 arg0, @OriginalArg(1) int arg1, @OriginalArg(2) int arg2, @OriginalArg(3) byte[] arg3, @OriginalArg(4) int arg4) {
		super(arg0, 3552, arg1, arg2, false);
		this.anInt6391 = arg2;
		@Pc(12) opengl local12 = arg0.anOpengl1;
		this.aClass164_Sub1_42.method3603(this);
		local12.glPixelStorei(3317, 1);
		local12.glTexImage1D(this.anInt6388, 0, this.anInt6389, this.anInt6391, 0, arg4, 5121, ByteBuffer.wrap(arg3));
		local12.glPixelStorei(3317, 4);
		this.method5556(true);
	}

	@OriginalMember(owner = "client!wq", name = "a", descriptor = "()V")
	@Override
	public void method5548() {
	}

	@OriginalMember(owner = "client!wq", name = "a", descriptor = "(BZ)V")
	public void method5562() {
		@Pc(3) opengl local3 = this.aClass164_Sub1_42.anOpengl1;
		this.aClass164_Sub1_42.method3603(this);
		local3.glTexParameteri(this.anInt6388, 10242, 33071);
	}
}
