import org.openrs2.deob.annotation.OriginalArg;
import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;

@OriginalClass("client!uia")
public final class Class6_Sub26 extends Class6 {

	@OriginalMember(owner = "client!uia", name = "<init>", descriptor = "(Lclient!fca;)V")
	public Class6_Sub26(@OriginalArg(0) Class3_Sub22 arg0) {
		super(arg0);
		Static325.method4967(super.anInt10233);
	}

	@OriginalMember(owner = "client!uia", name = "<init>", descriptor = "(ILclient!fca;)V")
	public Class6_Sub26(@OriginalArg(0) int arg0, @OriginalArg(1) Class3_Sub22 arg1) {
		super(arg0, arg1);
		Static325.method4967(super.anInt10233);
	}

	@OriginalMember(owner = "client!uia", name = "b", descriptor = "(I)Z")
	public boolean method8572() {
		return super.aClass3_Sub22_31.method2838().method183() >= 96;
	}

	@OriginalMember(owner = "client!uia", name = "a", descriptor = "(B)I")
	@Override
	protected int method8962() {
		return super.aClass3_Sub22_31.method2838().method183() < 96 ? 0 : 2;
	}

	@OriginalMember(owner = "client!uia", name = "a", descriptor = "(Z)V")
	@Override
	public void method8964() {
		if (super.aClass3_Sub22_31.method2838().method183() < 96) {
			super.anInt10233 = 0;
		}
		if (super.anInt10233 < 0 || super.anInt10233 > 2) {
			super.anInt10233 = this.method8962();
		}
	}

	@OriginalMember(owner = "client!uia", name = "a", descriptor = "(IB)I")
	@Override
	public int method8960(@OriginalArg(0) int arg0) {
		return super.aClass3_Sub22_31.method2838().method183() < 96 ? 3 : 1;
	}

	@OriginalMember(owner = "client!uia", name = "b", descriptor = "(IB)V")
	@Override
	protected void method8961(@OriginalArg(0) int arg0) {
		super.anInt10233 = arg0;
		Static325.method4967(super.anInt10233);
	}

	@OriginalMember(owner = "client!uia", name = "c", descriptor = "(I)I")
	public int method8575() {
		return super.anInt10233;
	}
}
