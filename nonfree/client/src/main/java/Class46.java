import java.io.EOFException;
import java.io.File;
import java.io.IOException;
import java.io.RandomAccessFile;
import org.openrs2.deob.annotation.OriginalArg;
import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;
import org.openrs2.deob.annotation.Pc;

@OriginalClass("client!de")
public final class Class46 {

	@OriginalMember(owner = "client!de", name = "a", descriptor = "Ljava/io/File;")
	private File aFile1;

	@OriginalMember(owner = "client!de", name = "b", descriptor = "J")
	private long aLong58;

	@OriginalMember(owner = "client!de", name = "c", descriptor = "J")
	private long aLong59;

	@OriginalMember(owner = "client!de", name = "d", descriptor = "Ljava/io/RandomAccessFile;")
	private RandomAccessFile aRandomAccessFile1;

	@OriginalMember(owner = "client!de", name = "<init>", descriptor = "()V")
	private Class46() {
	}

	@OriginalMember(owner = "client!de", name = "a", descriptor = "([BIIB)I")
	public int method1365(@OriginalArg(0) byte[] arg0, @OriginalArg(1) int arg1, @OriginalArg(2) int arg2) throws IOException {
		@Pc(6) int local6 = this.aRandomAccessFile1.read(arg0, arg2, arg1);
		if (local6 > 0) {
			this.aLong58 += local6;
		}
		return local6;
	}

	@OriginalMember(owner = "client!de", name = "a", descriptor = "(III[B)V")
	public void method1366(@OriginalArg(0) int arg0, @OriginalArg(2) int arg1, @OriginalArg(3) byte[] arg2) throws IOException {
		if (this.aLong58 + (long) arg0 > this.aLong59) {
			this.aRandomAccessFile1.seek(this.aLong59);
			this.aRandomAccessFile1.write(1);
			throw new EOFException();
		} else {
			this.aRandomAccessFile1.write(arg2, arg1, arg0);
			this.aLong58 += arg0;
		}
	}

	@OriginalMember(owner = "client!de", name = "a", descriptor = "(I)V")
	public void method1367() throws IOException {
		if (this.aRandomAccessFile1 != null) {
			this.aRandomAccessFile1.close();
			this.aRandomAccessFile1 = null;
		}
	}

	@OriginalMember(owner = "client!de", name = "a", descriptor = "(B)J")
	public long method1368() throws IOException {
		return this.aRandomAccessFile1.length();
	}

	@OriginalMember(owner = "client!de", name = "b", descriptor = "(I)Ljava/io/File;")
	public File method1369() {
		return this.aFile1;
	}

	@OriginalMember(owner = "client!de", name = "finalize", descriptor = "()V")
	@Override
	public void finalize() throws Throwable {
		if (this.aRandomAccessFile1 != null) {
			System.out.println("Warning! fileondisk " + this.aFile1 + " not closed correctly using close(). Auto-closing instead. ");
			this.method1367();
		}
	}

	@OriginalMember(owner = "client!de", name = "a", descriptor = "(JB)V")
	public void method1370(@OriginalArg(0) long arg0) throws IOException {
		this.aRandomAccessFile1.seek(arg0);
		this.aLong58 = arg0;
	}
}
