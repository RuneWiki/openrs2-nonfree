import org.openrs2.deob.annotation.OriginalArg;
import org.openrs2.deob.annotation.OriginalMember;
import org.openrs2.deob.annotation.Pc;

public final class Static45 {

	@OriginalMember(owner = "client!hf", name = "H", descriptor = "[B")
	private static byte[] aByteArray8;

	@OriginalMember(owner = "client!hf", name = "J", descriptor = "I")
	public static int anInt1375;

	@OriginalMember(owner = "client!hf", name = "K", descriptor = "I")
	public static int anInt1376;

	@OriginalMember(owner = "client!hf", name = "M", descriptor = "[F")
	public static float[] aFloatArray2;

	@OriginalMember(owner = "client!hf", name = "O", descriptor = "[I")
	public static int[] anIntArray144;

	@OriginalMember(owner = "client!hf", name = "P", descriptor = "[F")
	public static float[] aFloatArray3;

	@OriginalMember(owner = "client!hf", name = "R", descriptor = "[I")
	public static int[] anIntArray145;

	@OriginalMember(owner = "client!hf", name = "S", descriptor = "[F")
	public static float[] aFloatArray4;

	@OriginalMember(owner = "client!hf", name = "T", descriptor = "[F")
	public static float[] aFloatArray5;

	@OriginalMember(owner = "client!hf", name = "U", descriptor = "[Lclient!ed;")
	public static Class18[] aClass18Array1;

	@OriginalMember(owner = "client!hf", name = "V", descriptor = "[I")
	public static int[] anIntArray146;

	@OriginalMember(owner = "client!hf", name = "W", descriptor = "[F")
	public static float[] aFloatArray6;

	@OriginalMember(owner = "client!hf", name = "ab", descriptor = "[F")
	public static float[] aFloatArray7;

	@OriginalMember(owner = "client!hf", name = "cb", descriptor = "[F")
	public static float[] aFloatArray9;

	@OriginalMember(owner = "client!hf", name = "eb", descriptor = "I")
	private static int anInt1380;

	@OriginalMember(owner = "client!hf", name = "fb", descriptor = "[Z")
	public static boolean[] aBooleanArray14;

	@OriginalMember(owner = "client!hf", name = "gb", descriptor = "[Lclient!rd;")
	public static Class60[] aClass60Array1;

	@OriginalMember(owner = "client!hf", name = "ib", descriptor = "I")
	private static int anInt1382;

	@OriginalMember(owner = "client!hf", name = "jb", descriptor = "[Lclient!pb;")
	public static Class55[] aClass55Array1;

	@OriginalMember(owner = "client!hf", name = "kb", descriptor = "[Lclient!e;")
	public static Class15[] aClass15Array1;

	@OriginalMember(owner = "client!hf", name = "Z", descriptor = "Z")
	private static boolean aBoolean76 = false;

	@OriginalMember(owner = "client!hf", name = "a", descriptor = "(Lclient!ef;)Z")
	private static boolean method864(@OriginalArg(0) Class16 arg0) {
		if (!aBoolean76) {
			@Pc(7) byte[] local7 = arg0.method566(0, 0);
			if (local7 == null) {
				return false;
			}
			method870(local7);
			aBoolean76 = true;
		}
		return true;
	}

	@OriginalMember(owner = "client!hf", name = "a", descriptor = "(Lclient!ef;II)Lclient!hf;")
	public static Class3_Sub10 method865(@OriginalArg(0) Class16 arg0, @OriginalArg(1) int arg1, @OriginalArg(2) int arg2) {
		if (method864(arg0)) {
			@Pc(16) byte[] local16 = arg0.method566(arg1, arg2);
			return local16 == null ? null : new Class3_Sub10(local16);
		} else {
			arg0.method559(arg1, arg2);
			return null;
		}
	}

	@OriginalMember(owner = "client!hf", name = "a", descriptor = "([BI)V")
	public static void method866(@OriginalArg(0) byte[] arg0) {
		aByteArray8 = arg0;
		anInt1382 = 0;
		anInt1380 = 0;
	}

	@OriginalMember(owner = "client!hf", name = "c", descriptor = "(I)F")
	public static float method868(@OriginalArg(0) int arg0) {
		@Pc(3) int local3 = arg0 & 0x1FFFFF;
		@Pc(7) int local7 = arg0 & Integer.MIN_VALUE;
		@Pc(13) int local13 = arg0 >> 21 & 0x3FF;
		if (local7 != 0) {
			local3 = -local3;
		}
		return (float) ((double) local3 * Math.pow(2.0D, (double) (local13 - 788)));
	}

	@OriginalMember(owner = "client!hf", name = "d", descriptor = "(I)I")
	public static int method869(@OriginalArg(0) int arg0) {
		@Pc(1) int local1 = 0;
		@Pc(3) int local3 = 0;
		@Pc(8) int local8;
		while (arg0 >= 8 - anInt1380) {
			local8 = 8 - anInt1380;
			@Pc(14) int local14 = (0x1 << local8) - 1;
			local1 += (aByteArray8[anInt1382] >> anInt1380 & local14) << local3;
			anInt1380 = 0;
			anInt1382++;
			local3 += local8;
			arg0 -= local8;
		}
		if (arg0 > 0) {
			local8 = (0x1 << arg0) - 1;
			local1 += (aByteArray8[anInt1382] >> anInt1380 & local8) << local3;
			anInt1380 += arg0;
		}
		return local1;
	}

	@OriginalMember(owner = "client!hf", name = "b", descriptor = "([B)V")
	private static void method870(@OriginalArg(0) byte[] arg0) {
		method866(arg0);
		anInt1376 = 0x1 << method869(4);
		anInt1375 = 0x1 << method869(4);
		aFloatArray3 = new float[anInt1375];
		@Pc(24) int local24;
		@Pc(28) int local28;
		@Pc(32) int local32;
		@Pc(36) int local36;
		@Pc(41) int local41;
		@Pc(86) int local86;
		@Pc(138) int local138;
		@Pc(191) int local191;
		for (@Pc(17) int local17 = 0; local17 < 2; local17++) {
			local24 = local17 == 0 ? anInt1376 : anInt1375;
			local28 = local24 >> 1;
			local32 = local24 >> 2;
			local36 = local24 >> 3;
			@Pc(39) float[] local39 = new float[local28];
			for (local41 = 0; local41 < local32; local41++) {
				local39[local41 * 2] = (float) Math.cos((double) (local41 * 4) * 3.141592653589793D / (double) local24);
				local39[local41 * 2 + 1] = -((float) Math.sin((double) (local41 * 4) * 3.141592653589793D / (double) local24));
			}
			@Pc(84) float[] local84 = new float[local28];
			for (local86 = 0; local86 < local32; local86++) {
				local84[local86 * 2] = (float) Math.cos((double) (local86 * 2 + 1) * 3.141592653589793D / (double) (local24 * 2));
				local84[local86 * 2 + 1] = (float) Math.sin((double) (local86 * 2 + 1) * 3.141592653589793D / (double) (local24 * 2));
			}
			@Pc(136) float[] local136 = new float[local32];
			for (local138 = 0; local138 < local36; local138++) {
				local136[local138 * 2] = (float) Math.cos((double) (local138 * 4 + 2) * 3.141592653589793D / (double) local24);
				local136[local138 * 2 + 1] = -((float) Math.sin((double) (local138 * 4 + 2) * 3.141592653589793D / (double) local24));
			}
			@Pc(185) int[] local185 = new int[local36];
			local191 = Static111.method2012(local36 - 1);
			for (@Pc(193) int local193 = 0; local193 < local36; local193++) {
				local185[local193] = Static44.method854(local193, local191);
			}
			if (local17 == 0) {
				aFloatArray5 = local39;
				aFloatArray7 = local84;
				aFloatArray2 = local136;
				anIntArray144 = local185;
			} else {
				aFloatArray4 = local39;
				aFloatArray9 = local84;
				aFloatArray6 = local136;
				anIntArray146 = local185;
			}
		}
		local24 = method869(8) + 1;
		aClass60Array1 = new Class60[local24];
		for (local28 = 0; local28 < local24; local28++) {
			aClass60Array1[local28] = new Class60();
		}
		local32 = method869(6) + 1;
		for (local36 = 0; local36 < local32; local36++) {
			method869(16);
		}
		@Pc(269) int local269 = method869(6) + 1;
		aClass15Array1 = new Class15[local269];
		for (local41 = 0; local41 < local269; local41++) {
			aClass15Array1[local41] = new Class15();
		}
		@Pc(290) int local290 = method869(6) + 1;
		aClass18Array1 = new Class18[local290];
		for (local86 = 0; local86 < local290; local86++) {
			aClass18Array1[local86] = new Class18();
		}
		@Pc(311) int local311 = method869(6) + 1;
		aClass55Array1 = new Class55[local311];
		for (local138 = 0; local138 < local311; local138++) {
			aClass55Array1[local138] = new Class55();
		}
		@Pc(332) int local332 = method869(6) + 1;
		aBooleanArray14 = new boolean[local332];
		anIntArray145 = new int[local332];
		for (local191 = 0; local191 < local332; local191++) {
			aBooleanArray14[local191] = method872() != 0;
			method869(16);
			method869(16);
			anIntArray145[local191] = method869(8);
		}
	}

	@OriginalMember(owner = "client!hf", name = "a", descriptor = "()V")
	public static void method871() {
		aByteArray8 = null;
		aClass60Array1 = null;
		aClass15Array1 = null;
		aClass18Array1 = null;
		aClass55Array1 = null;
		aBooleanArray14 = null;
		anIntArray145 = null;
		aFloatArray3 = null;
		aFloatArray5 = null;
		aFloatArray7 = null;
		aFloatArray2 = null;
		aFloatArray4 = null;
		aFloatArray9 = null;
		aFloatArray6 = null;
		anIntArray144 = null;
		anIntArray146 = null;
	}

	@OriginalMember(owner = "client!hf", name = "b", descriptor = "()I")
	public static int method872() {
		@Pc(7) int local7 = aByteArray8[anInt1382] >> anInt1380 & 0x1;
		anInt1380++;
		anInt1382 += anInt1380 >> 3;
		anInt1380 &= 0x7;
		return local7;
	}
}
