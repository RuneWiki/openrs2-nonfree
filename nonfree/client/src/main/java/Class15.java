import org.openrs2.deob.annotation.OriginalArg;
import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;
import org.openrs2.deob.annotation.Pc;

@OriginalClass("client!c")
public final class Class15 {

	@OriginalMember(owner = "client!c", name = "b", descriptor = "[I")
	private final int[] anIntArray28 = new int[256];

	@OriginalMember(owner = "client!c", name = "f", descriptor = "I")
	private int anInt373;

	@OriginalMember(owner = "client!c", name = "g", descriptor = "[I")
	private final int[] anIntArray29 = new int[256];

	@OriginalMember(owner = "client!c", name = "r", descriptor = "I")
	private int anInt377;

	@OriginalMember(owner = "client!c", name = "t", descriptor = "I")
	private int anInt378;

	@OriginalMember(owner = "client!c", name = "u", descriptor = "I")
	private int anInt379;

	@OriginalMember(owner = "client!c", name = "<init>", descriptor = "([I)V")
	public Class15(@OriginalArg(0) int[] arg0) {
		for (@Pc(13) int local13 = 0; local13 < arg0.length; local13++) {
			this.anIntArray29[local13] = arg0[local13];
		}
		this.method316();
	}

	@OriginalMember(owner = "client!c", name = "a", descriptor = "(Z)I")
	public int method315() {
		if (this.anInt379-- == 0) {
			this.method319();
			this.anInt379 = 255;
		}
		return this.anIntArray29[this.anInt379];
	}

	@OriginalMember(owner = "client!c", name = "b", descriptor = "(B)V")
	private void method316() {
		@Pc(8) int local8 = -1640531527;
		@Pc(10) int local10 = -1640531527;
		@Pc(12) int local12 = -1640531527;
		@Pc(14) int local14 = -1640531527;
		@Pc(16) int local16 = -1640531527;
		@Pc(18) int local18 = -1640531527;
		@Pc(20) int local20 = -1640531527;
		@Pc(21) int local21 = -1640531527;
		@Pc(23) int local23;
		for (local23 = 0; local23 < 4; local23++) {
			local21 ^= local20 << 11;
			local20 += local18;
			local16 += local21;
			local20 ^= local18 >>> 2;
			local14 += local20;
			local18 += local16;
			local18 ^= local16 << 8;
			local16 += local14;
			local12 += local18;
			local16 ^= local14 >>> 16;
			local10 += local16;
			local14 += local12;
			local14 ^= local12 << 10;
			local8 += local14;
			local12 += local10;
			local12 ^= local10 >>> 4;
			local21 += local12;
			local10 += local8;
			local10 ^= local8 << 8;
			local20 += local10;
			local8 += local21;
			local8 ^= local21 >>> 9;
			local18 += local8;
			local21 += local20;
		}
		for (local23 = 0; local23 < 256; local23 += 8) {
			local14 += this.anIntArray29[local23 + 4];
			local16 += this.anIntArray29[local23 + 3];
			local20 += this.anIntArray29[local23 + 1];
			local18 += this.anIntArray29[local23 + 2];
			local12 += this.anIntArray29[local23 + 5];
			local8 += this.anIntArray29[local23 + 7];
			local10 += this.anIntArray29[local23 + 6];
			local21 += this.anIntArray29[local23];
			local21 ^= local20 << 11;
			local20 += local18;
			local16 += local21;
			local20 ^= local18 >>> 2;
			local18 += local16;
			local14 += local20;
			local18 ^= local16 << 8;
			local16 += local14;
			local12 += local18;
			local16 ^= local14 >>> 16;
			local14 += local12;
			local10 += local16;
			local14 ^= local12 << 10;
			local8 += local14;
			local12 += local10;
			local12 ^= local10 >>> 4;
			local21 += local12;
			local10 += local8;
			local10 ^= local8 << 8;
			local20 += local10;
			local8 += local21;
			local8 ^= local21 >>> 9;
			local21 += local20;
			local18 += local8;
			this.anIntArray28[local23] = local21;
			this.anIntArray28[local23 + 1] = local20;
			this.anIntArray28[local23 + 2] = local18;
			this.anIntArray28[local23 + 3] = local16;
			this.anIntArray28[local23 + 4] = local14;
			this.anIntArray28[local23 + 5] = local12;
			this.anIntArray28[local23 + 6] = local10;
			this.anIntArray28[local23 + 7] = local8;
		}
		for (local23 = 0; local23 < 256; local23 += 8) {
			local8 += this.anIntArray28[local23 + 7];
			local10 += this.anIntArray28[local23 + 6];
			local14 += this.anIntArray28[local23 + 4];
			local20 += this.anIntArray28[local23 + 1];
			local12 += this.anIntArray28[local23 + 5];
			local21 += this.anIntArray28[local23];
			local18 += this.anIntArray28[local23 + 2];
			local16 += this.anIntArray28[local23 + 3];
			local21 ^= local20 << 11;
			local20 += local18;
			local20 ^= local18 >>> 2;
			local14 += local20;
			local16 += local21;
			local18 += local16;
			local18 ^= local16 << 8;
			local16 += local14;
			local12 += local18;
			local16 ^= local14 >>> 16;
			local10 += local16;
			local14 += local12;
			local14 ^= local12 << 10;
			local12 += local10;
			local12 ^= local10 >>> 4;
			local21 += local12;
			local8 += local14;
			local10 += local8;
			local10 ^= local8 << 8;
			local20 += local10;
			local8 += local21;
			local8 ^= local21 >>> 9;
			local18 += local8;
			local21 += local20;
			this.anIntArray28[local23] = local21;
			this.anIntArray28[local23 + 1] = local20;
			this.anIntArray28[local23 + 2] = local18;
			this.anIntArray28[local23 + 3] = local16;
			this.anIntArray28[local23 + 4] = local14;
			this.anIntArray28[local23 + 5] = local12;
			this.anIntArray28[local23 + 6] = local10;
			this.anIntArray28[local23 + 7] = local8;
		}
		this.method319();
		this.anInt379 = 256;
	}

	@OriginalMember(owner = "client!c", name = "a", descriptor = "(I)V")
	private void method319() {
		this.anInt378 += ++this.anInt373;
		for (@Pc(17) int local17 = 0; local17 < 256; local17++) {
			@Pc(23) int local23 = this.anIntArray28[local17];
			if ((local17 & 0x2) == 0) {
				if ((local17 & 0x1) == 0) {
					this.anInt377 ^= this.anInt377 << 13;
				} else {
					this.anInt377 ^= this.anInt377 >>> 6;
				}
			} else if ((local17 & 0x1) == 0) {
				this.anInt377 ^= this.anInt377 << 2;
			} else {
				this.anInt377 ^= this.anInt377 >>> 16;
			}
			this.anInt377 += this.anIntArray28[local17 + 128 & 0xFF];
			@Pc(110) int local110;
			this.anIntArray28[local17] = local110 = this.anInt378 + this.anIntArray28[local23 >> 2 & 0xFF] + this.anInt377;
			this.anIntArray29[local17] = this.anInt378 = local23 + this.anIntArray28[local110 >> 8 >> 2 & 0xFF];
		}
	}
}
