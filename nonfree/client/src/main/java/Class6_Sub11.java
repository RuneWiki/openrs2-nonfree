import org.openrs2.deob.annotation.OriginalArg;
import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;

@OriginalClass("client!iv")
public final class Class6_Sub11 extends Class6 {

	@OriginalMember(owner = "client!iv", name = "<init>", descriptor = "(ILclient!ah;)V")
	public Class6_Sub11(@OriginalArg(0) int arg0, @OriginalArg(1) Class2_Sub5 arg1) {
		super(arg0, arg1);
	}

	@OriginalMember(owner = "client!iv", name = "<init>", descriptor = "(Lclient!ah;)V")
	public Class6_Sub11(@OriginalArg(0) Class2_Sub5 arg0) {
		super(arg0);
	}

	@OriginalMember(owner = "client!iv", name = "b", descriptor = "(II)V")
	@Override
	protected void method8577(@OriginalArg(0) int arg0) {
		super.anInt10283 = arg0;
	}

	@OriginalMember(owner = "client!iv", name = "a", descriptor = "(II)I")
	@Override
	public int method8573(@OriginalArg(0) int arg0) {
		return Static421.method5634(super.aClass2_Sub5_50.aClass6_Sub21_2.method7280()) ? 1 : 3;
	}

	@OriginalMember(owner = "client!iv", name = "a", descriptor = "(I)I")
	@Override
	protected int method8572() {
		return 0;
	}

	@OriginalMember(owner = "client!iv", name = "b", descriptor = "(I)I")
	public int method3433() {
		return super.anInt10283;
	}

	@OriginalMember(owner = "client!iv", name = "b", descriptor = "(Z)Z")
	public boolean method3434() {
		return Static421.method5634(super.aClass2_Sub5_50.aClass6_Sub21_2.method7280());
	}

	@OriginalMember(owner = "client!iv", name = "a", descriptor = "(Z)V")
	@Override
	public void method8576() {
		if (super.aClass2_Sub5_50.aClass6_Sub21_2.method7279() && !Static421.method5634(super.aClass2_Sub5_50.aClass6_Sub21_2.method7280())) {
			super.anInt10283 = 0;
		}
		if (super.anInt10283 < 0 || super.anInt10283 > 2) {
			super.anInt10283 = this.method8572();
		}
	}
}
