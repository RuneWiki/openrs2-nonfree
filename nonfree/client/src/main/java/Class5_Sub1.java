import java.awt.Component;
import javax.sound.sampled.AudioFormat;
import javax.sound.sampled.AudioSystem;
import javax.sound.sampled.DataLine.Info;
import javax.sound.sampled.LineUnavailableException;
import javax.sound.sampled.SourceDataLine;
import org.openrs2.deob.annotation.OriginalArg;
import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;
import org.openrs2.deob.annotation.Pc;

@OriginalClass("client!jd")
public final class Class5_Sub1 extends Class5 {

	@OriginalMember(owner = "client!jd", name = "D", descriptor = "Ljavax/sound/sampled/AudioFormat;")
	private AudioFormat anAudioFormat1;

	@OriginalMember(owner = "client!jd", name = "N", descriptor = "I")
	private int anInt1762;

	@OriginalMember(owner = "client!jd", name = "O", descriptor = "Ljavax/sound/sampled/SourceDataLine;")
	private SourceDataLine aSourceDataLine1;

	@OriginalMember(owner = "client!jd", name = "P", descriptor = "[B")
	private byte[] aByteArray24;

	@OriginalMember(owner = "client!jd", name = "b", descriptor = "(I)V")
	@Override
	public void method1788(@OriginalArg(0) int arg0) throws LineUnavailableException {
		try {
			@Pc(20) Info local20 = new Info(SourceDataLine.class, this.anAudioFormat1, arg0 << (Static149.aBoolean131 ? 2 : 1));
			this.aSourceDataLine1 = (SourceDataLine) AudioSystem.getLine(local20);
			this.aSourceDataLine1.open();
			this.aSourceDataLine1.start();
			this.anInt1762 = arg0;
		} catch (@Pc(36) LineUnavailableException local36) {
			if (Static2.method21(arg0) == 1) {
				this.aSourceDataLine1 = null;
				throw local36;
			} else {
				this.method1788(Static110.method1890(arg0));
			}
		}
	}

	@OriginalMember(owner = "client!jd", name = "a", descriptor = "(Ljava/awt/Component;)V")
	@Override
	public void method1800(@OriginalArg(0) Component arg0) {
		this.anAudioFormat1 = new AudioFormat((float) Static22.anInt472, 16, Static149.aBoolean131 ? 2 : 1, true, false);
		this.aByteArray24 = new byte[0x100 << (Static149.aBoolean131 ? 2 : 1)];
	}

	@OriginalMember(owner = "client!jd", name = "d", descriptor = "()V")
	@Override
	protected void method1799() {
		this.aSourceDataLine1.flush();
	}

	@OriginalMember(owner = "client!jd", name = "a", descriptor = "()I")
	@Override
	protected int method1787() {
		return this.anInt1762 - (this.aSourceDataLine1.available() >> (Static149.aBoolean131 ? 2 : 1));
	}

	@OriginalMember(owner = "client!jd", name = "c", descriptor = "()V")
	@Override
	protected void method1797() {
		@Pc(1) short local1 = 256;
		if (Static149.aBoolean131) {
			local1 = 512;
		}
		for (@Pc(9) int local9 = 0; local9 < local1; local9++) {
			@Pc(17) int local17 = this.anIntArray210[local9];
			if ((local17 + 8388608 & 0xFF000000) != 0) {
				local17 = local17 >> 31 ^ 0x7FFFFF;
			}
			this.aByteArray24[local9 * 2] = (byte) (local17 >> 8);
			this.aByteArray24[local9 * 2 + 1] = (byte) (local17 >> 16);
		}
		this.aSourceDataLine1.write(this.aByteArray24, 0, local1 << 1);
	}

	@OriginalMember(owner = "client!jd", name = "b", descriptor = "()V")
	@Override
	protected void method1792() {
		if (this.aSourceDataLine1 != null) {
			this.aSourceDataLine1.close();
			this.aSourceDataLine1 = null;
		}
	}
}
