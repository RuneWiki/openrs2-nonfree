import java.awt.Component;
import javax.sound.sampled.AudioFormat;
import javax.sound.sampled.AudioSystem;
import javax.sound.sampled.DataLine.Info;
import javax.sound.sampled.LineUnavailableException;
import javax.sound.sampled.SourceDataLine;
import org.openrs2.deob.annotation.OriginalArg;
import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;
import org.openrs2.deob.annotation.Pc;

@OriginalClass("client!df")
public final class Class18_Sub1 extends Class18 {

	@OriginalMember(owner = "client!df", name = "N", descriptor = "I")
	private int anInt754;

	@OriginalMember(owner = "client!df", name = "O", descriptor = "Ljavax/sound/sampled/SourceDataLine;")
	private SourceDataLine aSourceDataLine1;

	@OriginalMember(owner = "client!df", name = "P", descriptor = "Ljavax/sound/sampled/AudioFormat;")
	private AudioFormat anAudioFormat1;

	@OriginalMember(owner = "client!df", name = "Q", descriptor = "[B")
	private byte[] aByteArray9;

	@OriginalMember(owner = "client!df", name = "c", descriptor = "()V")
	@Override
	protected void method1532() {
		@Pc(1) short local1 = 256;
		if (Static28.aBoolean44) {
			local1 = 512;
		}
		for (@Pc(9) int local9 = 0; local9 < local1; local9++) {
			@Pc(17) int local17 = this.anIntArray194[local9];
			if ((local17 + 8388608 & 0xFF000000) != 0) {
				local17 = local17 >> 31 ^ 0x7FFFFF;
			}
			this.aByteArray9[local9 * 2] = (byte) (local17 >> 8);
			this.aByteArray9[local9 * 2 + 1] = (byte) (local17 >> 16);
		}
		this.aSourceDataLine1.write(this.aByteArray9, 0, local1 << 1);
	}

	@OriginalMember(owner = "client!df", name = "d", descriptor = "()V")
	@Override
	protected void method1538() {
		if (this.aSourceDataLine1 != null) {
			this.aSourceDataLine1.close();
			this.aSourceDataLine1 = null;
		}
	}

	@OriginalMember(owner = "client!df", name = "a", descriptor = "()V")
	@Override
	protected void method1528() {
		this.aSourceDataLine1.flush();
	}

	@OriginalMember(owner = "client!df", name = "b", descriptor = "()I")
	@Override
	protected int method1529() {
		return this.anInt754 - (this.aSourceDataLine1.available() >> (Static28.aBoolean44 ? 2 : 1));
	}

	@OriginalMember(owner = "client!df", name = "d", descriptor = "(I)V")
	@Override
	public void method1542(@OriginalArg(0) int arg0) throws LineUnavailableException {
		try {
			@Pc(20) Info local20 = new Info(SourceDataLine.class, this.anAudioFormat1, arg0 << (Static28.aBoolean44 ? 2 : 1));
			this.aSourceDataLine1 = (SourceDataLine) AudioSystem.getLine(local20);
			this.aSourceDataLine1.open();
			this.aSourceDataLine1.start();
			this.anInt754 = arg0;
		} catch (@Pc(36) LineUnavailableException local36) {
			if (Static23.method491(arg0) == 1) {
				this.aSourceDataLine1 = null;
				throw local36;
			} else {
				this.method1542(Static52.method998(arg0));
			}
		}
	}

	@OriginalMember(owner = "client!df", name = "a", descriptor = "(Ljava/awt/Component;)V")
	@Override
	public void method1531(@OriginalArg(0) Component arg0) {
		this.anAudioFormat1 = new AudioFormat((float) Static7.anInt420, 16, Static28.aBoolean44 ? 2 : 1, true, false);
		this.aByteArray9 = new byte[0x100 << (Static28.aBoolean44 ? 2 : 1)];
	}
}
