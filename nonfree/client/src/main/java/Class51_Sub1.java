import java.awt.Component;
import java.awt.event.MouseWheelEvent;
import java.awt.event.MouseWheelListener;
import org.openrs2.deob.annotation.OriginalArg;
import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;
import org.openrs2.deob.annotation.Pc;

@OriginalClass("client!qh")
public final class Class51_Sub1 extends Class51 implements MouseWheelListener {

	@OriginalMember(owner = "client!qh", name = "k", descriptor = "I")
	private int anInt3298 = 0;

	@OriginalMember(owner = "client!qh", name = "a", descriptor = "(Ljava/awt/Component;B)V")
	@Override
	public void method2406(@OriginalArg(0) Component arg0) {
		arg0.removeMouseWheelListener(this);
	}

	@OriginalMember(owner = "client!qh", name = "a", descriptor = "(ILjava/awt/Component;)V")
	@Override
	public void method2403(@OriginalArg(1) Component arg0) {
		arg0.addMouseWheelListener(this);
	}

	@OriginalMember(owner = "client!qh", name = "b", descriptor = "(B)I")
	@Override
	public synchronized int method2405() {
		@Pc(10) int local10 = this.anInt3298;
		this.anInt3298 = 0;
		return local10;
	}

	@OriginalMember(owner = "client!qh", name = "mouseWheelMoved", descriptor = "(Ljava/awt/event/MouseWheelEvent;)V")
	@Override
	public synchronized void mouseWheelMoved(@OriginalArg(0) MouseWheelEvent arg0) {
		this.anInt3298 += arg0.getWheelRotation();
	}
}
