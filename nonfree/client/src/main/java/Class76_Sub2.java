import java.awt.Component;
import javax.sound.sampled.AudioFormat;
import javax.sound.sampled.AudioSystem;
import javax.sound.sampled.DataLine.Info;
import javax.sound.sampled.LineUnavailableException;
import javax.sound.sampled.SourceDataLine;
import org.openrs2.deob.annotation.OriginalArg;
import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;
import org.openrs2.deob.annotation.Pc;

@OriginalClass("client!gaa")
public final class Class76_Sub2 extends Class76 {

	@OriginalMember(owner = "client!gaa", name = "F", descriptor = "I")
	private int anInt2887;

	@OriginalMember(owner = "client!gaa", name = "H", descriptor = "Ljavax/sound/sampled/SourceDataLine;")
	private SourceDataLine aSourceDataLine1;

	@OriginalMember(owner = "client!gaa", name = "I", descriptor = "[B")
	private byte[] aByteArray15;

	@OriginalMember(owner = "client!gaa", name = "J", descriptor = "Ljavax/sound/sampled/AudioFormat;")
	private AudioFormat anAudioFormat1;

	@OriginalMember(owner = "client!gaa", name = "G", descriptor = "Z")
	private boolean aBoolean219 = false;

	@OriginalMember(owner = "client!gaa", name = "d", descriptor = "(I)V")
	@Override
	public void method2520(@OriginalArg(0) int arg0) throws LineUnavailableException {
		try {
			@Pc(20) Info local20 = new Info(SourceDataLine.class, this.anAudioFormat1, arg0 << (Static269.aBoolean372 ? 2 : 1));
			this.aSourceDataLine1 = (SourceDataLine) AudioSystem.getLine(local20);
			this.aSourceDataLine1.open();
			this.aSourceDataLine1.start();
			this.anInt2887 = arg0;
		} catch (@Pc(36) LineUnavailableException local36) {
			if (Static263.method3802(arg0) == 1) {
				this.aSourceDataLine1 = null;
				throw local36;
			} else {
				this.method2520(Static122.method2187(arg0));
			}
		}
	}

	@OriginalMember(owner = "client!gaa", name = "d", descriptor = "()I")
	@Override
	protected int method2518() {
		return this.anInt2887 - (this.aSourceDataLine1.available() >> (Static269.aBoolean372 ? 2 : 1));
	}

	@OriginalMember(owner = "client!gaa", name = "c", descriptor = "()V")
	@Override
	protected void method2512() {
		if (this.aSourceDataLine1 != null) {
			this.aSourceDataLine1.close();
			this.aSourceDataLine1 = null;
		}
	}

	@OriginalMember(owner = "client!gaa", name = "b", descriptor = "()V")
	@Override
	protected void method2510() throws LineUnavailableException {
		this.aSourceDataLine1.flush();
		if (!this.aBoolean219) {
			return;
		}
		this.aSourceDataLine1.close();
		this.aSourceDataLine1 = null;
		@Pc(34) Info local34 = new Info(SourceDataLine.class, this.anAudioFormat1, this.anInt2887 << (Static269.aBoolean372 ? 2 : 1));
		this.aSourceDataLine1 = (SourceDataLine) AudioSystem.getLine(local34);
		this.aSourceDataLine1.open();
		this.aSourceDataLine1.start();
	}

	@OriginalMember(owner = "client!gaa", name = "a", descriptor = "(Ljava/awt/Component;)V")
	@Override
	public void method2515(@OriginalArg(0) Component arg0) {
		@Pc(1) javax.sound.sampled.Mixer.Info[] local1 = AudioSystem.getMixerInfo();
		if (local1 != null) {
			for (@Pc(9) int local9 = 0; local9 < local1.length; local9++) {
				@Pc(17) javax.sound.sampled.Mixer.Info local17 = local1[local9];
				if (local17 != null) {
					@Pc(23) String local23 = local17.getName();
					if (local23 != null && local23.toLowerCase().indexOf("soundmax") >= 0) {
						this.aBoolean219 = true;
					}
				}
			}
		}
		this.anAudioFormat1 = new AudioFormat((float) Static304.anInt5650, 16, Static269.aBoolean372 ? 2 : 1, true, false);
		this.aByteArray15 = new byte[0x100 << (Static269.aBoolean372 ? 2 : 1)];
	}

	@OriginalMember(owner = "client!gaa", name = "a", descriptor = "()V")
	@Override
	protected void method2508() {
		@Pc(1) short local1 = 256;
		if (Static269.aBoolean372) {
			local1 = 512;
		}
		for (@Pc(9) int local9 = 0; local9 < local1; local9++) {
			@Pc(17) int local17 = this.anIntArray129[local9];
			if ((local17 + 8388608 & 0xFF000000) != 0) {
				local17 = local17 >> 31 ^ 0x7FFFFF;
			}
			this.aByteArray15[local9 * 2] = (byte) (local17 >> 8);
			this.aByteArray15[local9 * 2 + 1] = (byte) (local17 >> 16);
		}
		this.aSourceDataLine1.write(this.aByteArray15, 0, local1 << 1);
	}
}
