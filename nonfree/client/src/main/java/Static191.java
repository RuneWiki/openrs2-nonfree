import javax.media.opengl.GL;
import org.openrs2.deob.annotation.OriginalArg;
import org.openrs2.deob.annotation.OriginalMember;
import org.openrs2.deob.annotation.Pc;

public final class Static191 {

	@OriginalMember(owner = "client!nn", name = "a", descriptor = "I")
	private static int anInt4212;

	@OriginalMember(owner = "client!nn", name = "b", descriptor = "I")
	private static int anInt4213;

	@OriginalMember(owner = "client!nn", name = "c", descriptor = "I")
	private static int anInt4214;

	@OriginalMember(owner = "client!nn", name = "d", descriptor = "I")
	private static int anInt4215;

	@OriginalMember(owner = "client!nn", name = "e", descriptor = "I")
	private static int anInt4216;

	@OriginalMember(owner = "client!nn", name = "f", descriptor = "[[[I")
	private static int[][][] anIntArrayArrayArray8;

	@OriginalMember(owner = "client!nn", name = "g", descriptor = "[Z")
	private static boolean[] aBooleanArray20;

	@OriginalMember(owner = "client!nn", name = "i", descriptor = "[Lclient!fn;")
	public static Class59[] aClass59Array1;

	@OriginalMember(owner = "client!nn", name = "j", descriptor = "[I")
	private static int[] anIntArray365;

	@OriginalMember(owner = "client!nn", name = "k", descriptor = "I")
	private static int anInt4218;

	@OriginalMember(owner = "client!nn", name = "m", descriptor = "I")
	private static int anInt4219;

	@OriginalMember(owner = "client!nn", name = "n", descriptor = "[Z")
	private static boolean[] aBooleanArray21;

	@OriginalMember(owner = "client!nn", name = "o", descriptor = "I")
	private static int anInt4220;

	@OriginalMember(owner = "client!nn", name = "p", descriptor = "[I")
	private static int[] anIntArray366;

	@OriginalMember(owner = "client!nn", name = "h", descriptor = "I")
	public static int anInt4217 = 0;

	@OriginalMember(owner = "client!nn", name = "l", descriptor = "[F")
	private static float[] aFloatArray18 = new float[] { 0.0F, 0.0F, 0.0F, 1.0F };

	@OriginalMember(owner = "client!nn", name = "a", descriptor = "()V")
	public static void method3082() {
		@Pc(1) GL local1 = Static294.aGL1;
		@Pc(3) int local3;
		for (local3 = 0; local3 < 4; local3++) {
			@Pc(10) int local10 = local3 + 16388;
			local1.glLightfv(local10, 4608, new float[] { 0.0F, 0.0F, 0.0F, 1.0F }, 0);
			local1.glLightf(local10, 4616, 0.0F);
			local1.glLightf(local10, 4615, 0.0F);
		}
		for (local3 = 0; local3 < 4; local3++) {
			anIntArray365[local3] = -1;
			method3090(local3);
		}
	}

	@OriginalMember(owner = "client!nn", name = "a", descriptor = "(IIIIIIII)V")
	public static void method3083(@OriginalArg(0) int arg0, @OriginalArg(1) int arg1, @OriginalArg(2) int arg2, @OriginalArg(3) int arg3, @OriginalArg(4) int arg4, @OriginalArg(5) int arg5, @OriginalArg(6) int arg6, @OriginalArg(7) int arg7) {
		if (!Static65.aBoolean96 || anInt4216 == arg3 && anInt4219 == arg4 && anInt4220 == arg5 && anInt4218 == arg6 && anInt4213 == arg7) {
			return;
		}
		@Pc(20) int local20;
		for (local20 = 0; local20 < 4; local20++) {
			aBooleanArray20[local20] = false;
		}
		local20 = 0;
		@Pc(33) int local33 = 0;
		@Pc(35) int local35;
		@Pc(40) int local40;
		label112: for (local35 = arg4; local35 <= arg6; local35++) {
			label110: for (local40 = arg5; local40 <= arg7; local40++) {
				@Pc(51) int local51 = anIntArrayArrayArray8[arg3][local35][local40];
				while (true) {
					while (true) {
						label96: while (true) {
							if (local51 == 0) {
								continue label110;
							}
							@Pc(59) int local59 = (local51 & 0xFF) - 1;
							local51 >>>= 0x8;
							@Pc(65) int local65;
							for (local65 = 0; local65 < local33; local65++) {
								if (local59 == anIntArray366[local65]) {
									continue label96;
								}
							}
							for (local65 = 0; local65 < 4; local65++) {
								if (local59 == anIntArray365[local65]) {
									if (!aBooleanArray20[local65]) {
										aBooleanArray20[local65] = true;
										local20++;
										if (local20 == 4) {
											break label112;
										}
									}
									continue label96;
								}
							}
							anIntArray366[local33++] = local59;
							local20++;
							if (local20 == 4) {
								break label112;
							}
						}
					}
				}
			}
		}
		for (local35 = 0; local35 < local33; local35++) {
			for (local40 = 0; local40 < 4; local40++) {
				if (!aBooleanArray20[local40]) {
					anIntArray365[local40] = anIntArray366[local35];
					aBooleanArray20[local40] = true;
					method3093(local40, aClass59Array1[anIntArray366[local35]], arg0, arg1, arg2);
					break;
				}
			}
		}
		for (local35 = 0; local35 < 4; local35++) {
			if (!aBooleanArray20[local35]) {
				anIntArray365[local35] = -1;
				method3090(local35);
			}
		}
		anInt4216 = arg3;
		anInt4219 = arg4;
		anInt4220 = arg5;
		anInt4218 = arg6;
		anInt4213 = arg7;
	}

	@OriginalMember(owner = "client!nn", name = "b", descriptor = "()V")
	public static void method3084() {
		aClass59Array1 = new Class59[255];
		anIntArray365 = new int[4];
		aBooleanArray21 = new boolean[4];
		anIntArray366 = new int[4];
		aBooleanArray20 = new boolean[4];
		anIntArrayArrayArray8 = new int[anInt4212][anInt4214][anInt4215];
	}

	@OriginalMember(owner = "client!nn", name = "a", descriptor = "(IZ)V")
	public static void method3085(@OriginalArg(0) int arg0, @OriginalArg(1) boolean arg1) {
		for (@Pc(1) int local1 = 0; local1 < anInt4217; local1++) {
			aClass59Array1[local1].method1354(arg1, arg0);
		}
		anInt4216 = -1;
		anInt4219 = -1;
		anInt4220 = -1;
		anInt4218 = -1;
		anInt4213 = -1;
	}

	@OriginalMember(owner = "client!nn", name = "c", descriptor = "()V")
	public static void method3086() {
		for (@Pc(1) int local1 = 0; local1 < 4; local1++) {
			anIntArray365[local1] = -1;
			method3090(local1);
		}
	}

	@OriginalMember(owner = "client!nn", name = "d", descriptor = "()V")
	public static void method3087() {
		anInt4217 = 0;
		for (@Pc(3) int local3 = 0; local3 < anInt4212; local3++) {
			for (@Pc(8) int local8 = 0; local8 < anInt4214; local8++) {
				for (@Pc(13) int local13 = 0; local13 < anInt4215; local13++) {
					anIntArrayArrayArray8[local3][local8][local13] = 0;
				}
			}
		}
	}

	@OriginalMember(owner = "client!nn", name = "a", descriptor = "(IIIIIII)V")
	public static void method3088(@OriginalArg(0) int arg0, @OriginalArg(1) int arg1, @OriginalArg(2) int arg2, @OriginalArg(3) int arg3, @OriginalArg(4) int arg4, @OriginalArg(5) int arg5, @OriginalArg(6) int arg6) {
		if (!Static65.aBoolean96) {
			return;
		}
		if (arg0 == 1 && arg5 > 0) {
			method3095(arg1, arg2, arg3, arg4, arg5 - 1, arg6);
		} else if (arg0 == 4 && arg5 < anInt4214 - 1) {
			method3095(arg1, arg2, arg3, arg4, arg5 + 1, arg6);
		} else if (arg0 == 8 && arg6 > 0) {
			method3095(arg1, arg2, arg3, arg4, arg5, arg6 - 1);
		} else if (arg0 == 2 && arg6 < anInt4215 - 1) {
			method3095(arg1, arg2, arg3, arg4, arg5, arg6 + 1);
		} else if (arg0 == 16 && arg5 > 0 && arg6 < anInt4215 - 1) {
			method3095(arg1, arg2, arg3, arg4, arg5 - 1, arg6 + 1);
		} else if (arg0 == 32 && arg5 < anInt4214 - 1 && arg6 < anInt4215 - 1) {
			method3095(arg1, arg2, arg3, arg4, arg5 + 1, arg6 + 1);
		} else if (arg0 == 128 && arg5 > 0 && arg6 > 0) {
			method3095(arg1, arg2, arg3, arg4, arg5 - 1, arg6 - 1);
		} else if (arg0 == 64 && arg5 < anInt4214 - 1 && arg6 > 0) {
			method3095(arg1, arg2, arg3, arg4, arg5 + 1, arg6 - 1);
		}
	}

	@OriginalMember(owner = "client!nn", name = "a", descriptor = "(I)V")
	private static void method3090(@OriginalArg(0) int arg0) {
		if (aBooleanArray21[arg0]) {
			aBooleanArray21[arg0] = false;
			@Pc(14) int local14 = arg0 + 16384 + 4;
			@Pc(16) GL local16 = Static294.aGL1;
			local16.glDisable(local14);
		}
	}

	@OriginalMember(owner = "client!nn", name = "a", descriptor = "(II[[[Lclient!aa;)V")
	public static void method3091(@OriginalArg(0) int arg0, @OriginalArg(1) int arg1, @OriginalArg(2) Class1_Sub2[][][] arg2) {
		if (!Static65.aBoolean96) {
			return;
		}
		@Pc(4) GL local4 = Static294.aGL1;
		Static1.method4721(0, 0);
		Static294.method4488(0);
		Static294.method4512();
		Static294.method4486(Static294.anInt5881);
		local4.glDepthMask(false);
		Static294.method4496(false);
		local4.glBlendFunc(774, 1);
		local4.glFogfv(2918, new float[] { 0.0F, 0.0F, 0.0F, 0.0F }, 0);
		local4.glTexEnvi(8960, 34176, 34166);
		local4.glTexEnvi(8960, 34192, 770);
		label71: for (@Pc(56) int local56 = 0; local56 < anInt4217; local56++) {
			@Pc(63) Class59 local63 = aClass59Array1[local56];
			@Pc(66) int local66 = local63.anInt1851;
			if (local63.aBoolean128) {
				local66--;
			}
			if (local63.aClass189_1 != null) {
				@Pc(76) int local76 = 0;
				@Pc(84) int local84 = (local63.anInt1847 >> 7) - local63.anInt1842;
				@Pc(92) int local92 = (local63.anInt1847 >> 7) + local63.anInt1842;
				if (local92 >= Static163.anInt3564) {
					local92 = Static163.anInt3564 - 1;
				}
				if (local84 < Static151.anInt3397) {
					local76 = Static151.anInt3397 - local84;
					local84 = Static151.anInt3397;
				}
				for (@Pc(112) int local112 = local84; local112 <= local92; local112++) {
					@Pc(121) short local121 = local63.aShortArray59[local76++];
					@Pc(133) int local133 = (local63.anInt1852 >> 7) + (local121 >> 8) - local63.anInt1842;
					@Pc(141) int local141 = local133 + (local121 & 0xFF) - 1;
					if (local133 < Static194.anInt4264) {
						local133 = Static194.anInt4264;
					}
					if (local141 >= Static235.anInt4964) {
						local141 = Static235.anInt4964 - 1;
					}
					for (@Pc(155) int local155 = local133; local155 <= local141; local155++) {
						@Pc(160) Class1_Sub2 local160 = null;
						if (local66 >= 0) {
							local160 = arg2[local66][local155][local112];
						}
						if (local66 < 0 || local160 != null && local160.aBoolean4) {
							Static294.method4511(201.5F - (float) local63.anInt1851 * 50.0F - 1.5F);
							local4.glTexEnvfv(8960, 8705, new float[] { 0.0F, 0.0F, 0.0F, local63.aFloat23 }, 0);
							local63.aClass189_1.method4639();
							continue label71;
						}
					}
				}
			}
		}
		local4.glTexEnvi(8960, 34176, 5890);
		local4.glTexEnvi(8960, 34192, 768);
		local4.glBlendFunc(770, 771);
		local4.glDepthMask(true);
		local4.glFogfv(2918, Static180.aFloatArray17, 0);
		local4.glEnableClientState(32888);
		Static294.method4500();
	}

	@OriginalMember(owner = "client!nn", name = "a", descriptor = "(IIIII)V")
	public static void method3092(@OriginalArg(0) int arg0, @OriginalArg(1) int arg1, @OriginalArg(2) int arg2, @OriginalArg(3) int arg3, @OriginalArg(4) int arg4) {
		if (!Static65.aBoolean96) {
			return;
		}
		label43: for (@Pc(4) int local4 = 0; local4 < 4; local4++) {
			if (anIntArray365[local4] != -1) {
				@Pc(20) int local20 = anIntArrayArrayArray8[arg0][arg1][arg2];
				@Pc(28) int local28;
				while (local20 != 0) {
					local28 = (local20 & 0xFF) - 1;
					local20 >>>= 0x8;
					if (local28 == anIntArray365[local4]) {
						continue label43;
					}
				}
				local20 = anIntArrayArrayArray8[arg0][arg3][arg4];
				while (local20 != 0) {
					local28 = (local20 & 0xFF) - 1;
					local20 >>>= 0x8;
					if (local28 == anIntArray365[local4]) {
						continue label43;
					}
				}
			}
			anIntArray365[local4] = -1;
			method3090(local4);
		}
	}

	@OriginalMember(owner = "client!nn", name = "a", descriptor = "(ILclient!fn;III)V")
	private static void method3093(@OriginalArg(0) int arg0, @OriginalArg(1) Class59 arg1, @OriginalArg(2) int arg2, @OriginalArg(3) int arg3, @OriginalArg(4) int arg4) {
		@Pc(5) int local5 = arg0 + 16384 + 4;
		@Pc(7) GL local7 = Static294.aGL1;
		if (!aBooleanArray21[arg0]) {
			local7.glEnable(local5);
			aBooleanArray21[arg0] = true;
		}
		local7.glLightf(local5, 4617, arg1.aFloat22);
		local7.glLightfv(local5, 4609, arg1.aFloatArray12, 0);
		aFloatArray18[0] = arg1.anInt1852 - arg2;
		aFloatArray18[1] = arg1.anInt1848 - arg3;
		aFloatArray18[2] = arg1.anInt1847 - arg4;
		local7.glLightfv(local5, 4611, aFloatArray18, 0);
	}

	@OriginalMember(owner = "client!nn", name = "a", descriptor = "(Lclient!fn;)V")
	public static void method3094(@OriginalArg(0) Class59 arg0) {
		if (anInt4217 >= 255) {
			System.out.println("Number of lights added exceeds maximum!");
		} else {
			aClass59Array1[anInt4217++] = arg0;
		}
	}

	@OriginalMember(owner = "client!nn", name = "a", descriptor = "(IIIIII)V")
	public static void method3095(@OriginalArg(0) int arg0, @OriginalArg(1) int arg1, @OriginalArg(2) int arg2, @OriginalArg(3) int arg3, @OriginalArg(4) int arg4, @OriginalArg(5) int arg5) {
		if (!Static65.aBoolean96 || anInt4216 == arg3 && anInt4219 == arg4 && anInt4220 == arg5 && anInt4218 == arg4 && anInt4213 == arg5) {
			return;
		}
		@Pc(20) int local20;
		for (local20 = 0; local20 < 4; local20++) {
			aBooleanArray20[local20] = false;
		}
		local20 = 0;
		@Pc(39) int local39 = anIntArrayArrayArray8[arg3][arg4][arg5];
		while (true) {
			@Pc(47) int local47;
			@Pc(53) int local53;
			label72: while (local39 != 0) {
				local47 = (local39 & 0xFF) - 1;
				local39 >>>= 0x8;
				for (local53 = 0; local53 < 4; local53++) {
					if (local47 == anIntArray365[local53]) {
						aBooleanArray20[local53] = true;
						continue label72;
					}
				}
				anIntArray366[local20++] = local47;
			}
			for (local47 = 0; local47 < local20; local47++) {
				for (local53 = 0; local53 < 4; local53++) {
					if (!aBooleanArray20[local53]) {
						anIntArray365[local53] = anIntArray366[local47];
						aBooleanArray20[local53] = true;
						method3093(local53, aClass59Array1[anIntArray366[local47]], arg0, arg1, arg2);
						break;
					}
				}
			}
			for (local47 = 0; local47 < 4; local47++) {
				if (!aBooleanArray20[local47]) {
					anIntArray365[local47] = -1;
					method3090(local47);
				}
			}
			anInt4216 = arg3;
			anInt4219 = arg4;
			anInt4220 = arg5;
			anInt4218 = arg4;
			anInt4213 = arg5;
			return;
		}
	}

	@OriginalMember(owner = "client!nn", name = "a", descriptor = "(III)V")
	public static void method3096() {
		anInt4212 = 4;
		anInt4214 = 104;
		anInt4215 = 104;
		anIntArrayArrayArray8 = new int[anInt4212][anInt4214][anInt4215];
	}

	@OriginalMember(owner = "client!nn", name = "f", descriptor = "()V")
	public static void method3097() {
		for (@Pc(1) int local1 = 0; local1 < anInt4217; local1++) {
			@Pc(8) Class59 local8 = aClass59Array1[local1];
			@Pc(11) int local11 = local8.anInt1851;
			if (local8.aBoolean127) {
				local11 = 0;
			}
			@Pc(19) int local19 = local8.anInt1851;
			if (local8.aBoolean126) {
				local19 = 3;
			}
			for (@Pc(26) int local26 = local11; local26 <= local19; local26++) {
				@Pc(31) int local31 = 0;
				@Pc(39) int local39 = (local8.anInt1847 >> 7) - local8.anInt1842;
				if (local39 < 0) {
					local31 = -local39;
					local39 = 0;
				}
				@Pc(55) int local55 = (local8.anInt1847 >> 7) + local8.anInt1842;
				if (local55 > anInt4215 - 1) {
					local55 = anInt4215 - 1;
				}
				for (@Pc(66) int local66 = local39; local66 <= local55; local66++) {
					@Pc(75) short local75 = local8.aShortArray59[local31++];
					@Pc(87) int local87 = (local8.anInt1852 >> 7) + (local75 >> 8) - local8.anInt1842;
					@Pc(95) int local95 = local87 + (local75 & 0xFF) - 1;
					if (local87 < 0) {
						local87 = 0;
					}
					if (local95 > anInt4214 - 1) {
						local95 = anInt4214 - 1;
					}
					for (@Pc(110) int local110 = local87; local110 <= local95; local110++) {
						@Pc(121) int local121 = anIntArrayArrayArray8[local26][local110][local66];
						if ((local121 & 0xFF) == 0) {
							anIntArrayArrayArray8[local26][local110][local66] = local121 | local1 + 1;
						} else if ((local121 & 0xFF00) == 0) {
							anIntArrayArrayArray8[local26][local110][local66] = local121 | local1 + 1 << 8;
						} else if ((local121 & 0xFF0000) == 0) {
							anIntArrayArrayArray8[local26][local110][local66] = local121 | local1 + 1 << 16;
						} else if ((local121 & 0xFF000000) == 0) {
							anIntArrayArrayArray8[local26][local110][local66] = local121 | local1 + 1 << 24;
						}
					}
				}
			}
		}
	}

	@OriginalMember(owner = "client!nn", name = "g", descriptor = "()V")
	public static void method3098() {
		aClass59Array1 = null;
		anIntArray365 = null;
		aBooleanArray21 = null;
		anIntArray366 = null;
		aBooleanArray20 = null;
		anIntArrayArrayArray8 = null;
	}
}
