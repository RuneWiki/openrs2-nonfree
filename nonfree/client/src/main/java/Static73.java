import org.openrs2.deob.annotation.OriginalArg;
import org.openrs2.deob.annotation.OriginalMember;
import org.openrs2.deob.annotation.Pc;

public final class Static73 {

	@OriginalMember(owner = "client!jc", name = "w", descriptor = "[Lclient!ie;")
	public static Class38[] aClass38Array1;

	@OriginalMember(owner = "client!jc", name = "y", descriptor = "[I")
	public static int[] anIntArray324;

	@OriginalMember(owner = "client!jc", name = "z", descriptor = "[F")
	public static float[] aFloatArray2;

	@OriginalMember(owner = "client!jc", name = "A", descriptor = "[F")
	public static float[] aFloatArray3;

	@OriginalMember(owner = "client!jc", name = "B", descriptor = "[B")
	private static byte[] aByteArray29;

	@OriginalMember(owner = "client!jc", name = "C", descriptor = "[F")
	public static float[] aFloatArray4;

	@OriginalMember(owner = "client!jc", name = "E", descriptor = "I")
	private static int anInt1956;

	@OriginalMember(owner = "client!jc", name = "F", descriptor = "[F")
	public static float[] aFloatArray5;

	@OriginalMember(owner = "client!jc", name = "J", descriptor = "[Z")
	public static boolean[] aBooleanArray8;

	@OriginalMember(owner = "client!jc", name = "K", descriptor = "[Lclient!rd;")
	public static Class71[] aClass71Array1;

	@OriginalMember(owner = "client!jc", name = "L", descriptor = "[F")
	public static float[] aFloatArray6;

	@OriginalMember(owner = "client!jc", name = "M", descriptor = "[Lclient!uf;")
	public static Class83[] aClass83Array1;

	@OriginalMember(owner = "client!jc", name = "O", descriptor = "[F")
	public static float[] aFloatArray8;

	@OriginalMember(owner = "client!jc", name = "P", descriptor = "[I")
	public static int[] anIntArray325;

	@OriginalMember(owner = "client!jc", name = "R", descriptor = "[I")
	public static int[] anIntArray326;

	@OriginalMember(owner = "client!jc", name = "S", descriptor = "[Lclient!fc;")
	public static Class28[] aClass28Array1;

	@OriginalMember(owner = "client!jc", name = "W", descriptor = "I")
	private static int anInt1961;

	@OriginalMember(owner = "client!jc", name = "X", descriptor = "I")
	public static int anInt1962;

	@OriginalMember(owner = "client!jc", name = "Y", descriptor = "I")
	public static int anInt1963;

	@OriginalMember(owner = "client!jc", name = "Z", descriptor = "[F")
	public static float[] aFloatArray9;

	@OriginalMember(owner = "client!jc", name = "I", descriptor = "Z")
	private static boolean aBoolean88 = false;

	@OriginalMember(owner = "client!jc", name = "b", descriptor = "([B)V")
	private static void method1405(@OriginalArg(0) byte[] arg0) {
		method1412(arg0);
		anInt1963 = 0x1 << method1408(4);
		anInt1962 = 0x1 << method1408(4);
		aFloatArray6 = new float[anInt1962];
		@Pc(24) int local24;
		@Pc(28) int local28;
		@Pc(32) int local32;
		@Pc(36) int local36;
		@Pc(41) int local41;
		@Pc(86) int local86;
		@Pc(138) int local138;
		@Pc(191) int local191;
		for (@Pc(17) int local17 = 0; local17 < 2; local17++) {
			local24 = local17 == 0 ? anInt1963 : anInt1962;
			local28 = local24 >> 1;
			local32 = local24 >> 2;
			local36 = local24 >> 3;
			@Pc(39) float[] local39 = new float[local28];
			for (local41 = 0; local41 < local32; local41++) {
				local39[local41 * 2] = (float) Math.cos((double) (local41 * 4) * 3.141592653589793D / (double) local24);
				local39[local41 * 2 + 1] = -((float) Math.sin((double) (local41 * 4) * 3.141592653589793D / (double) local24));
			}
			@Pc(84) float[] local84 = new float[local28];
			for (local86 = 0; local86 < local32; local86++) {
				local84[local86 * 2] = (float) Math.cos((double) (local86 * 2 + 1) * 3.141592653589793D / (double) (local24 * 2));
				local84[local86 * 2 + 1] = (float) Math.sin((double) (local86 * 2 + 1) * 3.141592653589793D / (double) (local24 * 2));
			}
			@Pc(136) float[] local136 = new float[local32];
			for (local138 = 0; local138 < local36; local138++) {
				local136[local138 * 2] = (float) Math.cos((double) (local138 * 4 + 2) * 3.141592653589793D / (double) local24);
				local136[local138 * 2 + 1] = -((float) Math.sin((double) (local138 * 4 + 2) * 3.141592653589793D / (double) local24));
			}
			@Pc(185) int[] local185 = new int[local36];
			local191 = Static129.method2201(local36 - 1);
			for (@Pc(193) int local193 = 0; local193 < local36; local193++) {
				local185[local193] = Static70.method1370(local191, local193);
			}
			if (local17 == 0) {
				aFloatArray2 = local39;
				aFloatArray5 = local84;
				aFloatArray3 = local136;
				anIntArray326 = local185;
			} else {
				aFloatArray8 = local39;
				aFloatArray4 = local84;
				aFloatArray9 = local136;
				anIntArray325 = local185;
			}
		}
		local24 = method1408(8) + 1;
		aClass28Array1 = new Class28[local24];
		for (local28 = 0; local28 < local24; local28++) {
			aClass28Array1[local28] = new Class28();
		}
		local32 = method1408(6) + 1;
		for (local36 = 0; local36 < local32; local36++) {
			method1408(16);
		}
		@Pc(269) int local269 = method1408(6) + 1;
		aClass38Array1 = new Class38[local269];
		for (local41 = 0; local41 < local269; local41++) {
			aClass38Array1[local41] = new Class38();
		}
		@Pc(290) int local290 = method1408(6) + 1;
		aClass71Array1 = new Class71[local290];
		for (local86 = 0; local86 < local290; local86++) {
			aClass71Array1[local86] = new Class71();
		}
		@Pc(311) int local311 = method1408(6) + 1;
		aClass83Array1 = new Class83[local311];
		for (local138 = 0; local138 < local311; local138++) {
			aClass83Array1[local138] = new Class83();
		}
		@Pc(332) int local332 = method1408(6) + 1;
		aBooleanArray8 = new boolean[local332];
		anIntArray324 = new int[local332];
		for (local191 = 0; local191 < local332; local191++) {
			aBooleanArray8[local191] = method1406() != 0;
			method1408(16);
			method1408(16);
			anIntArray324[local191] = method1408(8);
		}
	}

	@OriginalMember(owner = "client!jc", name = "a", descriptor = "()I")
	public static int method1406() {
		@Pc(7) int local7 = aByteArray29[anInt1956] >> anInt1961 & 0x1;
		anInt1961++;
		anInt1956 += anInt1961 >> 3;
		anInt1961 &= 0x7;
		return local7;
	}

	@OriginalMember(owner = "client!jc", name = "e", descriptor = "(I)F")
	public static float method1407(@OriginalArg(0) int arg0) {
		@Pc(3) int local3 = arg0 & 0x1FFFFF;
		@Pc(7) int local7 = arg0 & Integer.MIN_VALUE;
		@Pc(13) int local13 = arg0 >> 21 & 0x3FF;
		if (local7 != 0) {
			local3 = -local3;
		}
		return (float) ((double) local3 * Math.pow(2.0D, (double) (local13 - 788)));
	}

	@OriginalMember(owner = "client!jc", name = "f", descriptor = "(I)I")
	public static int method1408(@OriginalArg(0) int arg0) {
		@Pc(1) int local1 = 0;
		@Pc(3) int local3 = 0;
		@Pc(8) int local8;
		while (arg0 >= 8 - anInt1961) {
			local8 = 8 - anInt1961;
			@Pc(14) int local14 = (0x1 << local8) - 1;
			local1 += (aByteArray29[anInt1956] >> anInt1961 & local14) << local3;
			anInt1961 = 0;
			anInt1956++;
			local3 += local8;
			arg0 -= local8;
		}
		if (arg0 > 0) {
			local8 = (0x1 << arg0) - 1;
			local1 += (aByteArray29[anInt1956] >> anInt1961 & local8) << local3;
			anInt1961 += arg0;
		}
		return local1;
	}

	@OriginalMember(owner = "client!jc", name = "a", descriptor = "(Lclient!ke;II)Lclient!jc;")
	public static Class2_Sub11 method1409(@OriginalArg(0) Class43 arg0, @OriginalArg(1) int arg1, @OriginalArg(2) int arg2) {
		if (method1411(arg0)) {
			@Pc(16) byte[] local16 = arg0.method2205(arg2, arg1);
			return local16 == null ? null : new Class2_Sub11(local16);
		} else {
			arg0.method2220(arg2, arg1);
			return null;
		}
	}

	@OriginalMember(owner = "client!jc", name = "b", descriptor = "()V")
	public static void method1410() {
		aByteArray29 = null;
		aClass28Array1 = null;
		aClass38Array1 = null;
		aClass71Array1 = null;
		aClass83Array1 = null;
		aBooleanArray8 = null;
		anIntArray324 = null;
		aFloatArray6 = null;
		aFloatArray2 = null;
		aFloatArray5 = null;
		aFloatArray3 = null;
		aFloatArray8 = null;
		aFloatArray4 = null;
		aFloatArray9 = null;
		anIntArray326 = null;
		anIntArray325 = null;
	}

	@OriginalMember(owner = "client!jc", name = "a", descriptor = "(Lclient!ke;)Z")
	private static boolean method1411(@OriginalArg(0) Class43 arg0) {
		if (!aBoolean88) {
			@Pc(7) byte[] local7 = arg0.method2205(0, 0);
			if (local7 == null) {
				return false;
			}
			method1405(local7);
			aBoolean88 = true;
		}
		return true;
	}

	@OriginalMember(owner = "client!jc", name = "a", descriptor = "([BI)V")
	public static void method1412(@OriginalArg(0) byte[] arg0) {
		aByteArray29 = arg0;
		anInt1956 = 0;
		anInt1961 = 0;
	}
}
