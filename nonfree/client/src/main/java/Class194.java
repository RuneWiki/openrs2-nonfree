import org.openrs2.deob.annotation.OriginalArg;
import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;
import org.openrs2.deob.annotation.Pc;

@OriginalClass("client!lk")
public final class Class194 {

	@OriginalMember(owner = "client!lk", name = "a", descriptor = "I")
	private int anInt6114;

	@OriginalMember(owner = "client!lk", name = "l", descriptor = "I")
	private int anInt6123;

	@OriginalMember(owner = "client!lk", name = "b", descriptor = "[[I")
	private int[][] anIntArrayArray21;

	@OriginalMember(owner = "client!lk", name = "<init>", descriptor = "(II)V")
	public Class194(@OriginalArg(0) int arg0, @OriginalArg(1) int arg1) {
		if (arg0 != arg1) {
			@Pc(16) int local16 = Static605.method8488(arg0, arg1);
			@Pc(20) int local20 = arg1 / local16;
			@Pc(24) int local24 = arg0 / local16;
			this.anInt6114 = local24;
			this.anInt6123 = local20;
			this.anIntArrayArray21 = new int[local24][14];
			for (@Pc(37) int local37 = 0; local37 < local24; local37++) {
				@Pc(44) int[] local44 = this.anIntArrayArray21[local37];
				@Pc(52) double local52 = (double) local37 / (double) local24 + 6.0D;
				@Pc(60) int local60 = (int) Math.floor(local52 + 1.0D - 7.0D);
				if (local60 < 0) {
					local60 = 0;
				}
				@Pc(73) int local73 = (int) Math.ceil(local52 + 7.0D);
				if (local73 > 14) {
					local73 = 14;
				}
				@Pc(84) double local84 = (double) local20 / (double) local24;
				while (local60 < local73) {
					@Pc(93) double local93 = ((double) local60 - local52) * 3.141592653589793D;
					@Pc(95) double local95 = local84;
					if (local93 < -1.0E-4D || local93 > 1.0E-4D) {
						local95 = local84 * (Math.sin(local93) / local93);
					}
					local95 *= Math.cos(((double) local60 - local52) * 0.2243994752564138D) * 0.46D + 0.54D;
					local44[local60] = (int) Math.floor(local95 * 65536.0D + 0.5D);
					local60++;
				}
			}
		}
	}

	@OriginalMember(owner = "client!lk", name = "a", descriptor = "(II)I")
	public int method5326(@OriginalArg(0) int arg0) {
		if (this.anIntArrayArray21 != null) {
			arg0 = (int) ((long) arg0 * (long) this.anInt6123 / (long) this.anInt6114) + 6;
		}
		return arg0;
	}

	@OriginalMember(owner = "client!lk", name = "a", descriptor = "(B[S)[S")
	public short[] method5327(@OriginalArg(1) short[] arg0) {
		if (this.anIntArrayArray21 != null) {
			@Pc(28) int local28 = (int) ((long) this.anInt6123 * (long) arg0.length / (long) this.anInt6114) + 14;
			@Pc(31) int[] local31 = new int[local28];
			@Pc(33) int local33 = 0;
			@Pc(35) int local35 = 0;
			for (@Pc(37) int local37 = 0; local37 < arg0.length; local37++) {
				@Pc(43) short local43 = arg0[local37];
				@Pc(48) int[] local48 = this.anIntArrayArray21[local35];
				for (@Pc(50) int local50 = 0; local50 < 14; local50++) {
					local31[local33 + local50] += local48[local50] * local43 >> 2;
				}
				local35 += this.anInt6123;
				@Pc(84) int local84 = local35 / this.anInt6114;
				local33 += local84;
				local35 -= local84 * this.anInt6114;
			}
			arg0 = new short[local28];
			for (@Pc(109) int local109 = 0; local109 < local28; local109++) {
				@Pc(119) int local119 = local31[local109] + 8192 >> 14;
				if (local119 < -32768) {
					arg0[local109] = -32768;
				} else if (local119 > 32767) {
					arg0[local109] = 32767;
				} else {
					arg0[local109] = (short) local119;
				}
			}
		}
		return arg0;
	}

	@OriginalMember(owner = "client!lk", name = "a", descriptor = "(ZI)I")
	public int method5328(@OriginalArg(1) int arg0) {
		if (this.anIntArrayArray21 != null) {
			arg0 = (int) ((long) this.anInt6123 * (long) arg0 / (long) this.anInt6114);
		}
		return arg0;
	}

	@OriginalMember(owner = "client!lk", name = "a", descriptor = "([BZ)[B")
	public byte[] method5330(@OriginalArg(0) byte[] arg0) {
		if (this.anIntArrayArray21 != null) {
			@Pc(24) int local24 = (int) ((long) arg0.length * (long) this.anInt6123 / (long) this.anInt6114) + 14;
			@Pc(27) int[] local27 = new int[local24];
			@Pc(29) int local29 = 0;
			@Pc(31) int local31 = 0;
			for (@Pc(33) int local33 = 0; local33 < arg0.length; local33++) {
				@Pc(39) byte local39 = arg0[local33];
				@Pc(44) int[] local44 = this.anIntArrayArray21[local31];
				for (@Pc(46) int local46 = 0; local46 < 14; local46++) {
					local27[local46 + local29] += local39 * local44[local46];
				}
				local31 += this.anInt6123;
				@Pc(75) int local75 = local31 / this.anInt6114;
				local31 -= this.anInt6114 * local75;
				local29 += local75;
			}
			arg0 = new byte[local24];
			for (@Pc(100) int local100 = 0; local100 < local24; local100++) {
				@Pc(110) int local110 = local27[local100] + 32768 >> 16;
				if (local110 < -128) {
					arg0[local100] = -128;
				} else if (local110 > 127) {
					arg0[local100] = 127;
				} else {
					arg0[local100] = (byte) local110;
				}
			}
		}
		return arg0;
	}
}
