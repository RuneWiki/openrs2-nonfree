package com.jagex;

import org.openrs2.deob.annotation.OriginalArg;
import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;
import org.openrs2.deob.annotation.Pc;

@OriginalClass("client!lp")
public class Class402 {

	@OriginalMember(owner = "client!lp", name = "e", descriptor = "I")
	static final int anInt4680 = 128;

	@OriginalMember(owner = "client!lp", name = "n", descriptor = "I")
	static final int anInt4681 = 128;

	@OriginalMember(owner = "client!lp", name = "m", descriptor = "I")
	public static final int anInt4682 = 16;

	@OriginalMember(owner = "client!lp", name = "r", descriptor = "Ljava/lang/Object;")
	static Object anObject15;

	@OriginalMember(owner = "client!lp", name = "d", descriptor = "Ljava/lang/Object;")
	static Object anObject16;

	@OriginalMember(owner = "client!lp", name = "c", descriptor = "Ljava/lang/Object;")
	static Object anObject17;

	@OriginalMember(owner = "client!lp", name = "w", descriptor = "[Lclient!lz;")
	Interface38[] anInterface38Array2 = null;

	@OriginalMember(owner = "client!lp", name = "l", descriptor = "Lclient!ll;")
	Interface35 anInterface35_2 = null;

	@OriginalMember(owner = "client!lp", name = "u", descriptor = "[Lclient!lz;")
	public Interface38[] anInterface38Array1 = null;

	@OriginalMember(owner = "client!lp", name = "z", descriptor = "Lclient!ll;")
	public Interface35 anInterface35_3 = null;

	@OriginalMember(owner = "client!lp", name = "p", descriptor = "Lclient!ll;")
	public Interface35 anInterface35_1 = null;

	@OriginalMember(owner = "client!lp", name = "k", descriptor = "Lclient!afc;")
	Class104_Sub2 aClass104_Sub2_36;

	@OriginalMember(owner = "client!lp", name = "f", descriptor = "Z")
	public boolean aBoolean746;

	@OriginalMember(owner = "client!lp", name = "e", descriptor = "()V", line = 25)
	static void method28511() {
		@Pc(11) byte[] local11;
		if (anObject16 == null) {
			@Pc(5) Class140_Sub1_Sub2 local5 = new Class140_Sub1_Sub2();
			local11 = local5.method20324(128, 128, 16);
			anObject16 = Class113.method7411(local11, false);
		}
		if (anObject17 == null) {
			@Pc(22) Class140_Sub2_Sub2 local22 = new Class140_Sub2_Sub2();
			local11 = local22.method20409(128, 128, 16);
			anObject17 = Class113.method7411(local11, false);
		}
	}

	@OriginalMember(owner = "client!lp", name = "<init>", descriptor = "(Lclient!afc;)V", line = 37)
	Class402(@OriginalArg(0) Class104_Sub2 arg0) {
		this.aClass104_Sub2_36 = arg0;
		this.aBoolean746 = this.aClass104_Sub2_36.aBoolean471;
		if (this.aBoolean746 && !this.aClass104_Sub2_36.aBoolean469) {
			this.aBoolean746 = false;
		}
		if (this.aBoolean746 && !this.aClass104_Sub2_36.method21218(Class206.aClass206_21, Class226.aClass226_22)) {
			this.aBoolean746 = false;
		}
		if (this.aBoolean746 || this.aClass104_Sub2_36.method20954(Class206.aClass206_21, Class226.aClass226_22)) {
			method28511();
			if (this.aBoolean746) {
				@Pc(128) byte[] local128 = Class331.method27386(anObject16, false);
				this.anInterface35_2 = this.aClass104_Sub2_36.method21126(Class206.aClass206_21, 128, 128, 16, true, local128);
				@Pc(144) byte[] local144 = Class331.method27386(anObject17, false);
				this.anInterface35_3 = this.aClass104_Sub2_36.method21126(Class206.aClass206_21, 128, 128, 16, true, local144);
			} else {
				this.anInterface38Array2 = new Interface38[16];
				@Pc(66) int local66;
				@Pc(77) byte[] local77;
				for (local66 = 0; local66 < 16; local66++) {
					local77 = Class441.method28856(anObject16, local66 * 32768, 32768);
					this.anInterface38Array2[local66] = this.aClass104_Sub2_36.method20956(Class206.aClass206_21, 128, 128, true, local77);
				}
				this.anInterface38Array1 = new Interface38[16];
				for (local66 = 0; local66 < 16; local66++) {
					local77 = Class441.method28856(anObject17, local66 * 32768, 32768);
					this.anInterface38Array1[local66] = this.aClass104_Sub2_36.method20956(Class206.aClass206_21, 128, 128, true, local77);
				}
			}
		}
	}

	@OriginalMember(owner = "client!lp", name = "n", descriptor = "()Z", line = 65)
	public boolean method28508() {
		return this.aBoolean746 ? this.anInterface35_2 != null : this.anInterface38Array2 != null;
	}

	@OriginalMember(owner = "client!lp", name = "k", descriptor = "()Z", line = 65)
	public boolean method28514() {
		return this.aBoolean746 ? this.anInterface35_2 != null : this.anInterface38Array2 != null;
	}

	@OriginalMember(owner = "client!lp", name = "l", descriptor = "()Z", line = 69)
	public boolean method28509() {
		if (!this.aClass104_Sub2_36.aBoolean471) {
			return false;
		}
		if (this.anInterface35_1 == null) {
			@Pc(25) byte[] local25;
			if (anObject15 == null) {
				local25 = Class571.method31324(128, 128, 16, 8, new Class161_Sub1(419684), 4.0F, 4.0F, 16.0F, 0.5F, 0.6F);
				anObject15 = Class113.method7411(local25, false);
			}
			local25 = Class331.method27386(anObject15, false);
			@Pc(41) byte[] local41 = new byte[local25.length * 4];
			@Pc(43) int local43 = 0;
			for (@Pc(45) int local45 = 0; local45 < 16; local45++) {
				@Pc(52) int local52 = local45 * 16384;
				@Pc(54) int local54 = local52;
				for (@Pc(56) int local56 = 0; local56 < 128; local56++) {
					@Pc(65) int local65 = local54 + local56 * 128;
					@Pc(75) int local75 = local54 + (local56 - 1 & 0x7F) * 128;
					@Pc(85) int local85 = local54 + (local56 + 1 & 0x7F) * 128;
					for (@Pc(87) int local87 = 0; local87 < 128; local87++) {
						@Pc(107) float local107 = (float) ((local25[local75 + local87] & 0xFF) - (local25[local85 + local87] & 0xFF));
						@Pc(132) float local132 = (float) ((local25[local65 + (local87 - 1 & 0x7F)] & 0xFF) - (local25[local65 + (local87 + 1 & 0x7F)] & 0xFF));
						@Pc(147) float local147 = (float) (128.0D / Math.sqrt((double) (local132 * local132 + 16384.0F + local107 * local107)));
						local41[local43++] = (byte) (local132 * local147 + 127.0F);
						local41[local43++] = (byte) (local147 * 128.0F + 127.0F);
						local41[local43++] = (byte) (local107 * local147 + 127.0F);
						local41[local43++] = local25[local52++];
					}
				}
			}
			this.anInterface35_1 = this.aClass104_Sub2_36.method21126(Class206.aClass206_22, 128, 128, 16, true, local41);
		}
		return this.anInterface35_1 != null;
	}

	@OriginalMember(owner = "client!lp", name = "m", descriptor = "()Z", line = 69)
	public boolean method28510() {
		if (!this.aClass104_Sub2_36.aBoolean471) {
			return false;
		}
		if (this.anInterface35_1 == null) {
			@Pc(25) byte[] local25;
			if (anObject15 == null) {
				local25 = Class571.method31324(128, 128, 16, 8, new Class161_Sub1(419684), 4.0F, 4.0F, 16.0F, 0.5F, 0.6F);
				anObject15 = Class113.method7411(local25, false);
			}
			local25 = Class331.method27386(anObject15, false);
			@Pc(41) byte[] local41 = new byte[local25.length * 4];
			@Pc(43) int local43 = 0;
			for (@Pc(45) int local45 = 0; local45 < 16; local45++) {
				@Pc(52) int local52 = local45 * 16384;
				@Pc(54) int local54 = local52;
				for (@Pc(56) int local56 = 0; local56 < 128; local56++) {
					@Pc(65) int local65 = local54 + local56 * 128;
					@Pc(75) int local75 = local54 + (local56 - 1 & 0x7F) * 128;
					@Pc(85) int local85 = local54 + (local56 + 1 & 0x7F) * 128;
					for (@Pc(87) int local87 = 0; local87 < 128; local87++) {
						@Pc(107) float local107 = (float) ((local25[local75 + local87] & 0xFF) - (local25[local85 + local87] & 0xFF));
						@Pc(132) float local132 = (float) ((local25[local65 + (local87 - 1 & 0x7F)] & 0xFF) - (local25[local65 + (local87 + 1 & 0x7F)] & 0xFF));
						@Pc(147) float local147 = (float) (128.0D / Math.sqrt((double) (local132 * local132 + 16384.0F + local107 * local107)));
						local41[local43++] = (byte) (local132 * local147 + 127.0F);
						local41[local43++] = (byte) (local147 * 128.0F + 127.0F);
						local41[local43++] = (byte) (local107 * local147 + 127.0F);
						local41[local43++] = local25[local52++];
					}
				}
			}
			this.anInterface35_1 = this.aClass104_Sub2_36.method21126(Class206.aClass206_22, 128, 128, 16, true, local41);
		}
		return this.anInterface35_1 != null;
	}

	@OriginalMember(owner = "client!lp", name = "f", descriptor = "()Z", line = 69)
	public boolean method28512() {
		if (!this.aClass104_Sub2_36.aBoolean471) {
			return false;
		}
		if (this.anInterface35_1 == null) {
			@Pc(25) byte[] local25;
			if (anObject15 == null) {
				local25 = Class571.method31324(128, 128, 16, 8, new Class161_Sub1(419684), 4.0F, 4.0F, 16.0F, 0.5F, 0.6F);
				anObject15 = Class113.method7411(local25, false);
			}
			local25 = Class331.method27386(anObject15, false);
			@Pc(41) byte[] local41 = new byte[local25.length * 4];
			@Pc(43) int local43 = 0;
			for (@Pc(45) int local45 = 0; local45 < 16; local45++) {
				@Pc(52) int local52 = local45 * 16384;
				@Pc(54) int local54 = local52;
				for (@Pc(56) int local56 = 0; local56 < 128; local56++) {
					@Pc(65) int local65 = local54 + local56 * 128;
					@Pc(75) int local75 = local54 + (local56 - 1 & 0x7F) * 128;
					@Pc(85) int local85 = local54 + (local56 + 1 & 0x7F) * 128;
					for (@Pc(87) int local87 = 0; local87 < 128; local87++) {
						@Pc(107) float local107 = (float) ((local25[local75 + local87] & 0xFF) - (local25[local85 + local87] & 0xFF));
						@Pc(132) float local132 = (float) ((local25[local65 + (local87 - 1 & 0x7F)] & 0xFF) - (local25[local65 + (local87 + 1 & 0x7F)] & 0xFF));
						@Pc(147) float local147 = (float) (128.0D / Math.sqrt((double) (local132 * local132 + 16384.0F + local107 * local107)));
						local41[local43++] = (byte) (local132 * local147 + 127.0F);
						local41[local43++] = (byte) (local147 * 128.0F + 127.0F);
						local41[local43++] = (byte) (local107 * local147 + 127.0F);
						local41[local43++] = local25[local52++];
					}
				}
			}
			this.anInterface35_1 = this.aClass104_Sub2_36.method21126(Class206.aClass206_22, 128, 128, 16, true, local41);
		}
		return this.anInterface35_1 != null;
	}

	@OriginalMember(owner = "client!lp", name = "w", descriptor = "()Z", line = 69)
	public boolean method28513() {
		if (!this.aClass104_Sub2_36.aBoolean471) {
			return false;
		}
		if (this.anInterface35_1 == null) {
			@Pc(25) byte[] local25;
			if (anObject15 == null) {
				local25 = Class571.method31324(128, 128, 16, 8, new Class161_Sub1(419684), 4.0F, 4.0F, 16.0F, 0.5F, 0.6F);
				anObject15 = Class113.method7411(local25, false);
			}
			local25 = Class331.method27386(anObject15, false);
			@Pc(41) byte[] local41 = new byte[local25.length * 4];
			@Pc(43) int local43 = 0;
			for (@Pc(45) int local45 = 0; local45 < 16; local45++) {
				@Pc(52) int local52 = local45 * 16384;
				@Pc(54) int local54 = local52;
				for (@Pc(56) int local56 = 0; local56 < 128; local56++) {
					@Pc(65) int local65 = local54 + local56 * 128;
					@Pc(75) int local75 = local54 + (local56 - 1 & 0x7F) * 128;
					@Pc(85) int local85 = local54 + (local56 + 1 & 0x7F) * 128;
					for (@Pc(87) int local87 = 0; local87 < 128; local87++) {
						@Pc(107) float local107 = (float) ((local25[local75 + local87] & 0xFF) - (local25[local85 + local87] & 0xFF));
						@Pc(132) float local132 = (float) ((local25[local65 + (local87 - 1 & 0x7F)] & 0xFF) - (local25[local65 + (local87 + 1 & 0x7F)] & 0xFF));
						@Pc(147) float local147 = (float) (128.0D / Math.sqrt((double) (local132 * local132 + 16384.0F + local107 * local107)));
						local41[local43++] = (byte) (local132 * local147 + 127.0F);
						local41[local43++] = (byte) (local147 * 128.0F + 127.0F);
						local41[local43++] = (byte) (local107 * local147 + 127.0F);
						local41[local43++] = local25[local52++];
					}
				}
			}
			this.anInterface35_1 = this.aClass104_Sub2_36.method21126(Class206.aClass206_22, 128, 128, 16, true, local41);
		}
		return this.anInterface35_1 != null;
	}
}
