import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;
import org.openrs2.deob.annotation.Pc;

@OriginalClass("client!bd")
public final class Class3_Sub1_Sub2_Sub1 extends Class3_Sub1_Sub2 {

	@OriginalMember(owner = "client!bd", name = "hb", descriptor = "[I")
	public static int[] anIntArray38 = new int[512];

	@OriginalMember(owner = "client!bd", name = "gb", descriptor = "[I")
	public static int[] anIntArray37 = new int[2048];

	@OriginalMember(owner = "client!bd", name = "ob", descriptor = "[I")
	public static int[] anIntArray39 = new int[2048];

	@OriginalMember(owner = "client!bd", name = "rb", descriptor = "[I")
	public static int[] anIntArray41 = new int[2048];

	static {
		for (@Pc(29) int local29 = 1; local29 < 512; local29++) {
			anIntArray38[local29] = 32768 / local29;
		}
		for (@Pc(42) int local42 = 1; local42 < 2048; local42++) {
			anIntArray37[local42] = 65536 / local42;
		}
		for (@Pc(55) int local55 = 0; local55 < 2048; local55++) {
			anIntArray41[local55] = (int) (Math.sin((double) local55 * 0.0030679615D) * 65536.0D);
			anIntArray39[local55] = (int) (Math.cos((double) local55 * 0.0030679615D) * 65536.0D);
		}
	}
}
