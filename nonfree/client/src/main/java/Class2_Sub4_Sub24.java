import org.openrs2.deob.annotation.OriginalArg;
import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;
import org.openrs2.deob.annotation.Pc;

@OriginalClass("client!qga")
public final class Class2_Sub4_Sub24 extends Class2_Sub4 {

	@OriginalMember(owner = "client!qga", name = "H", descriptor = "I")
	private int anInt7019 = 4096;

	@OriginalMember(owner = "client!qga", name = "D", descriptor = "I")
	private int anInt7016 = 0;

	@OriginalMember(owner = "client!qga", name = "<init>", descriptor = "()V")
	public Class2_Sub4_Sub24() {
		super(1, false);
	}

	@OriginalMember(owner = "client!qga", name = "a", descriptor = "(II)[[I")
	@Override
	public int[][] method7561(@OriginalArg(1) int arg0) {
		@Pc(11) int[][] local11 = super.aClass143_41.method2531(arg0);
		if (super.aClass143_41.aBoolean214) {
			@Pc(29) int[][] local29 = this.method7567(arg0, 0);
			@Pc(33) int[] local33 = local29[0];
			@Pc(37) int[] local37 = local29[1];
			@Pc(41) int[] local41 = local29[2];
			@Pc(45) int[] local45 = local11[0];
			@Pc(49) int[] local49 = local11[1];
			@Pc(53) int[] local53 = local11[2];
			for (@Pc(55) int local55 = 0; local55 < Static143.anInt7434; local55++) {
				@Pc(61) int local61 = local33[local55];
				@Pc(65) int local65 = local37[local55];
				@Pc(69) int local69 = local41[local55];
				if (this.anInt7016 > local61) {
					local45[local55] = this.anInt7016;
				} else if (this.anInt7019 >= local61) {
					local45[local55] = local61;
				} else {
					local45[local55] = this.anInt7019;
				}
				if (this.anInt7016 > local65) {
					local49[local55] = this.anInt7016;
				} else if (this.anInt7019 >= local65) {
					local49[local55] = local65;
				} else {
					local49[local55] = this.anInt7019;
				}
				if (local69 < this.anInt7016) {
					local53[local55] = this.anInt7016;
				} else if (this.anInt7019 >= local69) {
					local53[local55] = local69;
				} else {
					local53[local55] = this.anInt7019;
				}
			}
		}
		return local11;
	}

	@OriginalMember(owner = "client!qga", name = "a", descriptor = "(ILclient!gk;B)V")
	@Override
	public void method7566(@OriginalArg(0) int arg0, @OriginalArg(1) Class2_Sub7 arg1) {
		if (arg0 == 0) {
			this.anInt7016 = arg1.method4518();
		} else if (arg0 == 1) {
			this.anInt7019 = arg1.method4518();
		} else if (arg0 == 2) {
			super.aBoolean789 = arg1.method4464() == 1;
		}
	}

	@OriginalMember(owner = "client!qga", name = "b", descriptor = "(II)[I")
	@Override
	public int[] method7564(@OriginalArg(0) int arg0) {
		@Pc(11) int[] local11 = super.aClass271_41.method5621(arg0);
		if (super.aClass271_41.aBoolean543) {
			@Pc(21) int[] local21 = this.method7568(arg0, 0);
			for (@Pc(23) int local23 = 0; local23 < Static143.anInt7434; local23++) {
				@Pc(29) int local29 = local21[local23];
				if (this.anInt7016 > local29) {
					local11[local23] = this.anInt7016;
				} else if (this.anInt7019 >= local29) {
					local11[local23] = local29;
				} else {
					local11[local23] = this.anInt7019;
				}
			}
		}
		return local11;
	}
}
