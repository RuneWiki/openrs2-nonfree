import org.openrs2.deob.annotation.OriginalArg;
import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;
import org.openrs2.deob.annotation.Pc;

@OriginalClass("client!fi")
public final class Class79 {

	@OriginalMember(owner = "client!fi", name = "a", descriptor = "I")
	private int anInt2302;

	@OriginalMember(owner = "client!fi", name = "f", descriptor = "[I")
	private int[] anIntArray161;

	@OriginalMember(owner = "client!fi", name = "h", descriptor = "[I")
	private int[] anIntArray162;

	@OriginalMember(owner = "client!fi", name = "i", descriptor = "I")
	private int anInt2308;

	@OriginalMember(owner = "client!fi", name = "j", descriptor = "I")
	private int anInt2309;

	@OriginalMember(owner = "client!fi", name = "l", descriptor = "I")
	private int anInt2311;

	@OriginalMember(owner = "client!fi", name = "<init>", descriptor = "()V")
	private Class79() {
	}

	@OriginalMember(owner = "client!fi", name = "<init>", descriptor = "([I)V")
	public Class79(@OriginalArg(0) int[] arg0) {
		this.anIntArray161 = new int[256];
		this.anIntArray162 = new int[256];
		for (@Pc(13) int local13 = 0; local13 < arg0.length; local13++) {
			this.anIntArray162[local13] = arg0[local13];
		}
		this.method1931();
	}

	@OriginalMember(owner = "client!fi", name = "a", descriptor = "(B)I")
	public int method1927() {
		if (this.anInt2308 == 0) {
			this.method1930();
			this.anInt2308 = 256;
		}
		return this.anIntArray162[--this.anInt2308];
	}

	@OriginalMember(owner = "client!fi", name = "b", descriptor = "(B)V")
	private void method1930() {
		this.anInt2309 += ++this.anInt2311;
		for (@Pc(27) int local27 = 0; local27 < 256; local27++) {
			@Pc(34) int local34 = this.anIntArray161[local27];
			if ((local27 & 0x2) == 0) {
				if ((local27 & 0x1) == 0) {
					this.anInt2302 ^= this.anInt2302 << 13;
				} else {
					this.anInt2302 ^= this.anInt2302 >>> 6;
				}
			} else if ((local27 & 0x1) == 0) {
				this.anInt2302 ^= this.anInt2302 << 2;
			} else {
				this.anInt2302 ^= this.anInt2302 >>> 16;
			}
			this.anInt2302 += this.anIntArray161[local27 + 128 & 0xFF];
			@Pc(130) int local130;
			this.anIntArray161[local27] = local130 = this.anInt2302 + this.anIntArray161[local34 >> 2 & 0xFF] + this.anInt2309;
			this.anIntArray162[local27] = this.anInt2309 = local34 + this.anIntArray161[local130 >> 8 >> 2 & 0xFF];
		}
	}

	@OriginalMember(owner = "client!fi", name = "b", descriptor = "(Z)V")
	private void method1931() {
		@Pc(8) int local8 = -1640531527;
		@Pc(10) int local10 = -1640531527;
		@Pc(12) int local12 = -1640531527;
		@Pc(14) int local14 = -1640531527;
		@Pc(16) int local16 = -1640531527;
		@Pc(18) int local18 = -1640531527;
		@Pc(20) int local20 = -1640531527;
		@Pc(21) int local21 = -1640531527;
		@Pc(23) int local23;
		for (local23 = 0; local23 < 4; local23++) {
			local21 ^= local20 << 11;
			local20 += local18;
			local16 += local21;
			local20 ^= local18 >>> 2;
			local14 += local20;
			local18 += local16;
			local18 ^= local16 << 8;
			local16 += local14;
			local12 += local18;
			local16 ^= local14 >>> 16;
			local14 += local12;
			local10 += local16;
			local14 ^= local12 << 10;
			local8 += local14;
			local12 += local10;
			local12 ^= local10 >>> 4;
			local21 += local12;
			local10 += local8;
			local10 ^= local8 << 8;
			local8 += local21;
			local20 += local10;
			local8 ^= local21 >>> 9;
			local18 += local8;
			local21 += local20;
		}
		for (local23 = 0; local23 < 256; local23 += 8) {
			local8 += this.anIntArray162[local23 + 7];
			local20 += this.anIntArray162[local23 + 1];
			local21 += this.anIntArray162[local23];
			local14 += this.anIntArray162[local23 + 4];
			local18 += this.anIntArray162[local23 + 2];
			local10 += this.anIntArray162[local23 + 6];
			local16 += this.anIntArray162[local23 + 3];
			local12 += this.anIntArray162[local23 + 5];
			local21 ^= local20 << 11;
			local20 += local18;
			local16 += local21;
			local20 ^= local18 >>> 2;
			local18 += local16;
			local14 += local20;
			local18 ^= local16 << 8;
			local12 += local18;
			local16 += local14;
			local16 ^= local14 >>> 16;
			local14 += local12;
			local10 += local16;
			local14 ^= local12 << 10;
			local12 += local10;
			local8 += local14;
			local12 ^= local10 >>> 4;
			local21 += local12;
			local10 += local8;
			local10 ^= local8 << 8;
			local20 += local10;
			local8 += local21;
			local8 ^= local21 >>> 9;
			local21 += local20;
			local18 += local8;
			this.anIntArray161[local23] = local21;
			this.anIntArray161[local23 + 1] = local20;
			this.anIntArray161[local23 + 2] = local18;
			this.anIntArray161[local23 + 3] = local16;
			this.anIntArray161[local23 + 4] = local14;
			this.anIntArray161[local23 + 5] = local12;
			this.anIntArray161[local23 + 6] = local10;
			this.anIntArray161[local23 + 7] = local8;
		}
		for (local23 = 0; local23 < 256; local23 += 8) {
			local12 += this.anIntArray161[local23 + 5];
			local18 += this.anIntArray161[local23 + 2];
			local16 += this.anIntArray161[local23 + 3];
			local8 += this.anIntArray161[local23 + 7];
			local10 += this.anIntArray161[local23 + 6];
			local14 += this.anIntArray161[local23 + 4];
			local20 += this.anIntArray161[local23 + 1];
			local21 += this.anIntArray161[local23];
			local21 ^= local20 << 11;
			local20 += local18;
			local16 += local21;
			local20 ^= local18 >>> 2;
			local14 += local20;
			local18 += local16;
			local18 ^= local16 << 8;
			local12 += local18;
			local16 += local14;
			local16 ^= local14 >>> 16;
			local14 += local12;
			local10 += local16;
			local14 ^= local12 << 10;
			local8 += local14;
			local12 += local10;
			local12 ^= local10 >>> 4;
			local10 += local8;
			local21 += local12;
			local10 ^= local8 << 8;
			local20 += local10;
			local8 += local21;
			local8 ^= local21 >>> 9;
			local18 += local8;
			local21 += local20;
			this.anIntArray161[local23] = local21;
			this.anIntArray161[local23 + 1] = local20;
			this.anIntArray161[local23 + 2] = local18;
			this.anIntArray161[local23 + 3] = local16;
			this.anIntArray161[local23 + 4] = local14;
			this.anIntArray161[local23 + 5] = local12;
			this.anIntArray161[local23 + 6] = local10;
			this.anIntArray161[local23 + 7] = local8;
		}
		this.method1930();
		this.anInt2308 = 256;
	}

	@OriginalMember(owner = "client!fi", name = "b", descriptor = "(I)I")
	public int method1934() {
		if (this.anInt2308 == 0) {
			this.method1930();
			this.anInt2308 = 256;
		}
		return this.anIntArray162[this.anInt2308 - 1];
	}
}
