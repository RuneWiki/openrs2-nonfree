import java.awt.Component;
import org.openrs2.deob.annotation.OriginalArg;
import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;

@OriginalClass("client!qf")
public interface Interface1 {

	@OriginalMember(owner = "client!qf", name = "a", descriptor = "(II)V")
	void method1462(@OriginalArg(1) int arg0);

	@OriginalMember(owner = "client!qf", name = "a", descriptor = "(Ljava/awt/Component;ZII)V")
	void method1463(@OriginalArg(0) Component arg0, @OriginalArg(1) boolean arg1, @OriginalArg(2) int arg2) throws Exception;

	@OriginalMember(owner = "client!qf", name = "a", descriptor = "(I[I)V")
	void method1464(@OriginalArg(0) int arg0, @OriginalArg(1) int[] arg1);

	@OriginalMember(owner = "client!qf", name = "a", descriptor = "(IB)I")
	int method1465(@OriginalArg(0) int arg0);

	@OriginalMember(owner = "client!qf", name = "b", descriptor = "(II)V")
	void method1466(@OriginalArg(1) int arg0);

	@OriginalMember(owner = "client!qf", name = "a", descriptor = "(III)V")
	void method1467(@OriginalArg(0) int arg0, @OriginalArg(2) int arg1) throws Exception;
}
