import org.openrs2.deob.annotation.OriginalArg;
import org.openrs2.deob.annotation.OriginalMember;
import org.openrs2.deob.annotation.Pc;

public final class Static99 {

	@OriginalMember(owner = "client!cv", name = "e", descriptor = "I")
	public static int anInt1773;

	@OriginalMember(owner = "client!cv", name = "b", descriptor = "F")
	public static float aFloat8;

	@OriginalMember(owner = "client!cv", name = "g", descriptor = "[[F")
	public static final float[][] aFloatArrayArray7 = new float[2][8];

	@OriginalMember(owner = "client!cv", name = "c", descriptor = "[[I")
	public static final int[][] anIntArrayArray9 = new int[2][8];

	@OriginalMember(owner = "client!cv", name = "a", descriptor = "(F)F")
	public static float method1524(@OriginalArg(0) float arg0) {
		@Pc(7) float local7 = (float) Math.pow(2.0D, (double) arg0) * 32.703197F;
		return local7 * 3.1415927F / 11025.0F;
	}
}
