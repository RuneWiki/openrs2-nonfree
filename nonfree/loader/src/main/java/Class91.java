import java.io.ByteArrayInputStream;
import java.io.ByteArrayOutputStream;
import java.io.IOException;
import java.util.Hashtable;
import java.util.zip.ZipEntry;
import java.util.zip.ZipInputStream;
import org.openrs2.deob.annotation.OriginalArg;
import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;
import org.openrs2.deob.annotation.Pc;

@OriginalClass("loader!unpack")
public class Class91 {

	@OriginalMember(owner = "loader!unpack", name = "cache", descriptor = "Ljava/util/Hashtable;")
	private final Hashtable aHashtable3 = new Hashtable();

	@OriginalMember(owner = "loader!unpack", name = "<init>", descriptor = "()V")
	public Class91() {
	}

	@OriginalMember(owner = "loader!unpack", name = "<init>", descriptor = "([B)V")
	public Class91(@OriginalArg(0) byte[] arg0) throws IOException {
		@Pc(9) byte[] local9 = new byte[1000];
		@Pc(17) ZipInputStream local17 = new ZipInputStream(new ByteArrayInputStream(arg0));
		while (true) {
			@Pc(25) String local25;
			do {
				@Pc(20) ZipEntry local20 = local17.getNextEntry();
				if (local20 == null) {
					local17.close();
					return;
				}
				local25 = local20.getName();
			} while (!local25.endsWith(".class"));
			local25 = local25.substring(0, local25.length() - 6);
			local25 = local25.replace('/', '.');
			@Pc(46) ByteArrayOutputStream local46 = new ByteArrayOutputStream();
			while (true) {
				@Pc(52) int local52 = local17.read(local9, 0, 1000);
				if (local52 == -1) {
					@Pc(65) byte[] local65 = local46.toByteArray();
					this.aHashtable3.put(local25, local65);
					break;
				}
				local46.write(local9, 0, local52);
			}
		}
	}

	@OriginalMember(owner = "loader!unpack", name = "a", descriptor = "(ILjava/lang/String;)[B")
	public final byte[] method2821(@OriginalArg(1) String arg0) {
		return (byte[]) this.aHashtable3.remove(arg0);
	}
}
